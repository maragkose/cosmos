...
<?php
	...
	$t_sponsor_table = config_get( 'cosmos_sponsorship_table' );
	...
	$query = "SELECT b.id as candidate, s.id as sponsor, s.paid, b.project_id, b.fixed_in_version, b.status 
		FROM $t_candidate_table b, $t_sponsor_table s 
		WHERE s.user_id=$t_user AND s.candidate_id = b.id $t_show_clause AND $t_project_clause
...
<table class="width100" cellspacing="1">
	...
	<tr>
		<td class="form-title" width="10%"><?php echo lang_get( 'email_candidate' ) ?></td>
...
<?php 
		...
		for ( $i=0; $i < $t_sponsors; ++$i ) {
			...
			$t_candidate = candidate_get( $row['candidate'] );
			$t_sponsor = sponsorship_get( $row['sponsor'] );
			
			# describe candidate
			$t_status = string_attribute( get_enum_element( 'status', $t_candidate->status ) );
			$t_resolution = string_attribute( get_enum_element( 'resolution', $t_candidate->resolution ) );
			$t_version_id = version_get_id( $t_candidate->fixed_in_version, $t_project );
			if ( ( false !== $t_version_id ) && ( VERSION_RELEASED == version_get_field( $t_version_id, 'released' ) ) ) {
				$t_released_label = '<a title="' . lang_get( 'released' ) . '">' . $t_candidate->fixed_in_version . '</a>';
			} else {
				$t_released_label = $t_candidate->fixed_in_version;
			}

			echo '<tr bgcolor="' . get_status_color( $t_candidate->status ) . '">';
			echo '<td><a href="' . string_get_candidate_view_url( $row['candidate'] ) . '">' . candidate_format_id( $row['candidate'] ) . '</a></td>';
			echo '<td>' . project_get_field( $t_candidate->project_id, 'name' ) . '&nbsp;</td>';
			echo '<td class="right">' . $t_released_label . '&nbsp;</td>';
			echo '<td><span class="issue-status" title="' . $t_resolution . '">' . $t_status . '</span></td>';
			echo '<td>';
			print_user( $t_candidate->handler_id );
			echo '</td>';

			# summary
			echo '<td>' . string_display_line( $t_candidate->summary );
			if ( VS_PRIVATE == $t_candidate->view_state ) {
...
<?php } # end sponsored issues
	...
	$query = "SELECT b.id as candidate, s.id as sponsor, s.paid, b.project_id, b.fixed_in_version, b.status 
		FROM $t_candidate_table b, $t_sponsor_table s 
		WHERE b.handler_id=$t_user AND s.candidate_id = b.id $t_show_clause AND $t_project_clause
		ORDER BY s.paid ASC, b.project_id ASC, b.fixed_in_version ASC, b.status ASC, b.id DESC";
...
<?php 
		...
		for ( $i=0; $i < $t_sponsors; ++$i ) {
			...
			$t_candidate = candidate_get( $row['candidate'] );
			$t_sponsor = sponsorship_get( $row['sponsor'] );
			$t_candidatelist[] = $row['candidate'] . ':' . $row['sponsor'];
			...
			echo '</td>';
			echo '<td class="right">' . sponsorship_format_amount( $t_sponsor->amount ) . '</td>';
			echo '<td><select name="sponsor_' . $row['candidate'] . '_' . $t_sponsor->id . '">';
		...
		$t_hidden_candidate_list = implode( ',', $t_candidatelist );
?>
<!-- Totals -->
<tr border="top">
	<td colspan="5"></td>
...
<tr>
	...
	<input type="hidden" name="candidatelist" value="<?php echo $t_hidden_candidate_list ?>" />
	<!-- BUTTONS -->
	<tr>
		<td colspan="5">&nbsp;</td>
		<!-- Update Button -->
