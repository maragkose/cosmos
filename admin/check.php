...
<?php
	...
	function test_candidate_download_threshold()
	{
		$t_pass = true;
	...
	function test_candidate_attachments_allow_flags()
	{
		$t_pass = true;
...
<html>
<head>
<title> COSMOS Administration - Check Installation </title>
...
<tr>
	<td bgcolor="#f0f0ff">
		...
		<p>COSMOS requires at least <b>PHP <?php echo PHP_MIN_VERSION ?></b>.  If you are not running this version or above, you or your administrator will need to upgrade your build of PHP.</p>
...
<?php
	...
	test_candidate_download_threshold();
	test_candidate_attachments_allow_flags();
		...
		print_test_row( '$g_candidate_link_tag is not empty ("' . config_get_global( 'candidate_link_tag' ) . '")',
				'' <> config_get_global( 'candidate_link_tag' ) );
		print_test_row( '$g_candidatenote_link_tag is not empty ("' . config_get_global( 'candidatenote_link_tag' ) . '")',
				'' <> config_get_global( 'candidatenote_link_tag' ) );
	...
	if ( ini_get_bool( 'register_globals' ) ) { ?>
		...
		<tr>
			<td bgcolor="#ffcc22">
				...
				You have register_globals enabled in PHP, which is considered a security risk.  Since version 0.18, COSMOS has no longer relied on register_globals being enabled.  PHP versions later that 4.2.0 have this option disabled by default.  For more information on the security issues associated with enabling register_globals, see <a href="http://www.php.net/manual/en/security.globals.php">this page</a>.

				If you have no other PHP applications that rely on register_globals, you should add the line <pre>register_globals = Off</pre> to your php.ini file;  if you do have other applications that require register_globals, you could consider disabling it for your COSMOS installation by adding the line <pre>php_value register_globals off</pre> to a <tt>.htaccess</tt> file or a <tt>&lt;Directory&gt;</tt> or <tt>&lt;Location&gt;</tt> block in your apache configuration file.  See the apache documentation if you require more information.
			</td>
		</tr>
		</table>
	...
	if ( CRYPT_FULL_SALT == config_get_global( 'login_method' ) ) { ?>
		...
		<tr>
			<td bgcolor="#ff0088">
				...
				You can simply change the login_method in your configuration file. You don't need to do anything else, even if you migrate to MD5 (which produces incompatible hashes). This is because COSMOS will automatically convert the passwords as users log in.
			</td>
		</tr>
		</table>
		...
		<tr>
			<td bgcolor="#ffcc22">
				...
				You can simply change the login_method in your configuration file to MD5. COSMOS will automatically convert the passwords as users log in.
			</td>
		</tr>
		</table>
...
<tr>
	<td bgcolor="#f4f4f4">
		...
		?>
				...
				COSMOS variable 'max_file_size': <?php echo config_get_global( 'max_file_size' ) ?> bytes</p>
		...
		?>
				<p>File uploads are DISABLED.  To enable them, make sure <tt>$g_file_uploads = on</tt> is in your php.ini file and <tt>allow_file_upload = ON</tt> is in your cosmos config file.</p>
		<?php
			}
		?>
	</td>
...
<tr>
	<td bgcolor="#f4f4f4">
		...
		<p>You can test the ability for COSMOS to send email notifications with this form.  Just click "Send Mail".  If the page takes a very long time to reappear or results in an error then you will need to investigate your php/mail server settings (see PHPMailer related settings in your config_inc.php, if they don't exist, copy from config_defaults_inc.php).  Note that errors can also appear in the server error log.  More help can be found at the <a href="http://www.php.net/manual/en/ref.mail.php">PHP website</a> if you are using the mail() PHPMailer sending mode.</p>
		<?php
		if ( $f_mail_test ) {
