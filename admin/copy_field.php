...
<html>
<head>
<title> COSMOS Administration - Copy Custom Fields to Built-in </title>
...
<table width="100%" border="0" cellspacing="0" cellpadding="0" bgcolor="#ffffff">
	<tr class="top-bar">
		...
		<td class="title">
			COSMOS Administration - Copy Custom Fields to Built-in
		</td>
	</tr>
</table>
<br /><br />
...
<?php
	...
	$t_string_table = config_get_global( 'cosmos_custom_field_string_table' );
	...
	if ( FALSE == $result ) {
		...
		for ( $i=0 ; $i < $count ; $i++ ) {
			...
			# trace candidate id back to project
			...
			printf("\n<tr %s><td><a href=\"../view.php?id=%d\">%07d</a></td><td>%s</td><td>",
					helper_alternate_class(), $v_candidate_id, $v_candidate_id, $v_value);
			...
			if ( $t_valid ) {
				...
				if ( ! candidate_set_field( $v_candidate_id, $f_dest_field, $t_cust_value ) ) {
