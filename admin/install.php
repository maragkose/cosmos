...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	@require_once( dirname( dirname( __FILE__ ) ) . DIRECTORY_SEPARATOR . 'core/custom_field_api.php' );
	...
	$f_database_name = gpc_get( 'database_name', config_get( 'database_name', 'candidatetrack') );
...
<html>
<head>
<title> COSMOS Administration - Installation  </title>
...
<tr>
	<td bgcolor="#ffffff">
		...
		<?php
			# due to a candidate in ADODB, this call prompts warnings, hence the @
...
<?php
	...
	if ( false == $g_failed ) {
		...
		if ( $f_log_queries ) {
			...
			echo 'INSERT INTO cosmos_config_table ( value, type, access_reqd, config_id, project_id, user_id ) VALUES (\'' . $lastid . '\', 1, 90, \'database_version\', 0, 0 );' . "\r\n";
...
<?php print_test( 'Checking for register_globals are off for cosmos', ! ini_get_bool( 'register_globals' ), false, 'change php.ini to disable register_globals setting' ) ?>
...
<tr>
	...
	<?php
		...
		if ( $t_result == true ) {
			...
			$project_id = project_create( 'Engineer', 'Default Job Order for Engineers', 10, VS_PUBLIC, '', true ); 
			#
			# Create additional Custom Fields. 
			#
			category_add( $project_id, 'Developer');
			category_add( $project_id, 'Tester');
			category_add( $project_id, 'Network Engineer');
			category_add( $project_id, 'Other');
			
			$field_id = custom_field_create('OverallTestScore');
			custom_field_link( $field_id, $project_id );
			custom_field_set_sequence( $field_id, $project_id, 10); 
			$field_id = custom_field_create('CTestResult');
			custom_field_link( $field_id, $project_id );
			custom_field_set_sequence( $field_id, $project_id, 20); 
			$field_id = custom_field_create('CplusplusTestResult');
			custom_field_link( $field_id, $project_id );
			custom_field_set_sequence( $field_id, $project_id, 30); 
			$field_id = custom_field_create('LinuxScriptingTestResult');
			custom_field_link( $field_id, $project_id );
			custom_field_set_sequence( $field_id, $project_id, 40); 
			$field_id = custom_field_create('NetworkingTestResult');
			custom_field_link( $field_id, $project_id );
			custom_field_set_sequence( $field_id, $project_id, 50); 
			
			$field_id = custom_field_create('1st Interview Date');
			custom_field_link( $field_id, $project_id );
			custom_field_set_sequence( $field_id, $project_id, 60); 
			$t_cosmos_custom_field_table = config_get( 'cosmos_custom_field_table' );
			$query = "UPDATE $t_cosmos_custom_field_table SET type=8 WHERE id=$field_id ";
			db_query( $query );
			
			$field_id = custom_field_create('1st Interview Time');
			custom_field_link( $field_id, $project_id );
			custom_field_set_sequence( $field_id, $project_id, 70); 
			
			$field_id = custom_field_create('Interview Room');
			custom_field_link( $field_id, $project_id );
			custom_field_set_sequence( $field_id, $project_id, 80); 
			
			$field_id = custom_field_create('2nd Interview Date');
			custom_field_link( $field_id, $project_id );
			custom_field_set_sequence( $field_id, $project_id, 90); 
			$query = "UPDATE $t_cosmos_custom_field_table SET type=8 WHERE id=$field_id ";
			db_query( $query );
		
			$field_id = custom_field_create('2nd Interview Time');
			custom_field_link( $field_id, $project_id );
			custom_field_set_sequence( $field_id, $project_id, 100); 
			print_test_result( GOOD );
		} else {
			print_test_result( BAD, false, 'Database user doesn\'t have access to the database ( ' .  db_error_msg() . ' )' );
		}
...
<tr>
	...
	<?php
		$t_cosmos_config_table = config_get_global( 'cosmos_config_table' );
		$t_query = "SELECT COUNT(*) FROM $t_cosmos_config_table";
...
<tr>
	...
	<?php
		$t_query = "INSERT INTO $t_cosmos_config_table ( value, type, access_reqd, config_id, project_id, user_id ) VALUES ('test', 1, 90, 'database_test', 20, 0 )";
		$t_result = @$g_db->Execute( $t_query );
...
<tr>
	...
	<?php
		$t_query = "UPDATE $t_cosmos_config_table SET value='test_update' WHERE config_id='database_test'";
		$t_result = @$g_db->Execute( $t_query );
...
<tr>
	...
	<?php
		$t_query = "DELETE FROM $t_cosmos_config_table WHERE config_id='database_test'";
		$t_result = @$g_db->Execute( $t_query );
...
<?php if ( $f_db_exists ) { ?>
<p><a href="../login_page.php">Continue</a> to log into COSMOS</p>
<?php } else { ?>
