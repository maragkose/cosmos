...
<?php
	...
	function upgrade_move_att2disk($p_source) {
		...
		if ( $p_source == 'attachment' ) {
				$t_file_table = config_get_global( 'cosmos_candidate_file_table' );
				$t_candidate_label = "Bug";
		}
		if ( $p_source == 'project' ) {
				$t_file_table = config_get_global( 'cosmos_project_file_table' );
				$t_candidate_label = "Project";
		}
		...
		if ( $count > 0 ) {
			...
			# Headings
			echo '<tr bgcolor="#ffffff"><th width="10%">' . $t_candidate_label . '</th><th width="20%">Attachment</th><th width="70%">Status</th></tr>';
		}
		...
		for ( $i=0 ; $i < $count ; $i++ ) {
			...
			// trace candidate id back to project to determine the proper file path
			if ( $p_source == 'attachment' ) {
				$t_project_id = candidate_get_field( $v_candidate_id, 'project_id' );
				$t_candidate_id = $v_candidate_id;
			} else { 
				$t_project_id = (int) $v_project_id;
				$t_candidate_id = $t_project_id;
			}
			...
			printf("\n<tr %s><td>%8d</td><td>%s</td><td>", helper_alternate_class(), $t_candidate_id, $v_filename);
...
<html>
<head>
<title>COSMOS Administration - Move <?php echo $t_type ?> to Disk</title>
