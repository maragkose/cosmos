...
<?php
	...
	$upgrades[] = new SQLUpgrade(
			...
			"INSERT INTO $t_project_table
			  ...
			  VALUES ( '0000001', 'mantis', 'development', 'on', 'public', 'Mantis.  Report problems with the actual candidate tracker here. (Do not remove this account.  You can set it to be disabled or private if you do not wish to see it)' )"
		);
	...
	$upgrades[] = new SQLUpgrade(
			'0.13-5',
			'Add project_id column to mantis_candidate_table',
			"ALTER TABLE $t_candidate_table ADD project_id INT (7) UNSIGNED ZEROFILL not null AFTER id"
		);

	$upgrades[] = new SQLUpgrade(
			'0.13-6',
			'Change category column in mantis_candidate_table to varchar',
			"ALTER TABLE $t_candidate_table CHANGE category category VARCHAR (32) not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.13-7',
			'Change version column in mantis_candidate_table to varchar',
			"ALTER TABLE $t_candidate_table CHANGE version version VARCHAR (32) DEFAULT 'none' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.13-8',
			'Set project_id to "0000001" for all candidates',
