<?php
# Mantis - a php based candidatetracking system
...
# None of these have descriptions...
	...
	$upgrades[] = new SQLUpgrade(
			'0.14a-3',
			'',
			"ALTER TABLE $t_candidate_table CHANGE eta eta VARCHAR (32) DEFAULT 'none' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14a-4',
			'',
			"ALTER TABLE $t_candidate_table CHANGE projection projection VARCHAR (32) DEFAULT 'none' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14a-5',
			'',
			"ALTER TABLE $t_candidate_table CHANGE resolution resolution VARCHAR (32) DEFAULT 'open' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14a-6',
			'',
			"ALTER TABLE $t_candidate_table CHANGE priority priority VARCHAR (32) DEFAULT 'none' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14a-7',
			'',
			"ALTER TABLE $t_candidate_table CHANGE status status VARCHAR (32) DEFAULT 'new' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14a-8',
			'',
			"ALTER TABLE $t_candidate_table CHANGE severity severity VARCHAR (32) DEFAULT 'minor' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14a-9',
			'',
			"ALTER TABLE $t_candidate_table CHANGE reproducibility reproducibility VARCHAR (32) DEFAULT 'always' not null"
		);
...
# Change some of the TIMESTAMP fields to DATETIME
	...
	$upgrades[] = new SQLUpgrade(
			'0.14-0',
			'Change some of the TIMESTAMP fields to DATETIME',
			"ALTER TABLE $t_candidate_table CHANGE date_submitted date_submitted DATETIME"
		);
	$upgrades[] = new SQLUpgrade(
			'0.14-1',
			'Change some of the TIMESTAMP fields to DATETIME',
			"ALTER TABLE $t_candidatenote_table CHANGE date_submitted date_submitted DATETIME"
		);
...
# ENUM Updates (Before ALTERation)
	...
	$upgrades[] = new SQLUpgrade(
			'0.14-34',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET eta='10' WHERE eta='none'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-35',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET eta='20' WHERE eta='< 1 day'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-36',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET eta='30' WHERE eta='2-3 days'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-37',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET eta='40' WHERE eta='< 1 week'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-38',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET eta='50' WHERE eta='< 1 month'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-39',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET eta='60' WHERE eta='> 1 month'"
		);
	...
	$upgrades[] = new SQLUpgrade(
			'0.14-40',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET projection='10' WHERE projection='none'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-41',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET projection='30' WHERE projection='tweak'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-42',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET projection='50' WHERE projection='minor fix'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-43',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET projection='70' WHERE projection='major rework'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-44',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET projection='90' WHERE projection='redesign'"
		);
	...
	$upgrades[] = new SQLUpgrade(
			'0.14-45',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET resolution='10' WHERE resolution='open'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-46',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET resolution='20' WHERE resolution='fixed'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-47',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET resolution='30' WHERE resolution='reopened'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-48',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET resolution='40' WHERE resolution='unable to duplicate'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-49',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET resolution='50' WHERE resolution='not fixable'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-50',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET resolution='60' WHERE resolution='duplicate'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-51',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET resolution='70' WHERE resolution='not a candidate'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-52',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET resolution='80' WHERE resolution='suspended'"
		);
	...
	$upgrades[] = new SQLUpgrade(
			'0.14-53',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET priority='10' WHERE priority='none'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-54',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET priority='20' WHERE priority='low'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-55',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET priority='30' WHERE priority='normal'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-56',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET priority='40' WHERE priority='high'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-57',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET priority='50' WHERE priority='urgent'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-58',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET priority='60' WHERE priority='immediate'"
		);
	...
	$upgrades[] = new SQLUpgrade(
			'0.14-59',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET status='10' WHERE status='new'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-60',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET status='20' WHERE status='feedback'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-61',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET status='30' WHERE status='acknowledged'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-62',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET status='40' WHERE status='confirmed'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-63',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET status='50' WHERE status='assigned'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-64',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET status='90' WHERE status='resolved'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-65',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET status='90' WHERE status='closed'"
		);
	...
	$upgrades[] = new SQLUpgrade(
			'0.14-66',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET severity='10' WHERE severity='feature'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-67',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET severity='20' WHERE severity='trivial'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-68',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET severity='30' WHERE severity='text'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-69',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET severity='40' WHERE severity='tweak'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-70',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET severity='50' WHERE severity='minor'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-71',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET severity='60' WHERE severity='major'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-72',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET severity='70' WHERE severity='crash'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-73',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET severity='80' WHERE severity='block'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-74',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET reproducibility='10' WHERE reproducibility='always'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-75',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET reproducibility='30' WHERE reproducibility='sometimes'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-76',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET reproducibility='50' WHERE reproducibility='random'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-77',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET reproducibility='70' WHERE reproducibility='have not tried'"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-78',
			'ENUM Updates (Before ALTERation)',
			"UPDATE $t_candidate_table SET reproducibility='90' WHERE reproducibility='unable to duplicate'"
		);
...
# Change ENUM to INT
	...
	$upgrades[] = new SQLUpgrade(
			'0.14-82',
			'Change ENUM to INT',
			"ALTER TABLE $t_candidate_table CHANGE eta eta INT (2) DEFAULT '10' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-83',
			'Change ENUM to INT',
			"ALTER TABLE $t_candidate_table CHANGE projection projection INT (2) DEFAULT '10' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-84',
			'Change ENUM to INT',
			"ALTER TABLE $t_candidate_table CHANGE resolution resolution INT (2) DEFAULT '10' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-85',
			'Change ENUM to INT',
			"ALTER TABLE $t_candidate_table CHANGE priority priority INT (2) DEFAULT '30' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-86',
			'Change ENUM to INT',
			"ALTER TABLE $t_candidate_table CHANGE status status INT (2) DEFAULT '10' not null"
		);
...
# Update dates to be legal
	...
	$upgrades[] = new SQLUpgrade(
			'0.14-88',
			'Update dates to be legal',
			"UPDATE $t_candidate_table SET date_submitted='1970-01-01 00:00:01' WHERE date_submitted='0000-00-00 00:00:00'"
		);
...
# Add new user prefs
	...
	$upgrades[] = new SQLUpgrade(
			'0.14-101',
			'Add new user prefs',
			"ALTER TABLE $t_user_pref_table ADD email_on_candidatenote INT (1) not null"
		);
...
# Make new candidate file table

	$upgrades[] = new SQLUpgrade(
			'0.14-111',
			'Make new candidate file table',
			"CREATE TABLE IF NOT EXISTS $t_candidate_file_table (
			  id int(7) unsigned zerofill DEFAULT '0000000' NOT NULL auto_increment,
			  candidate_id int(7) unsigned zerofill DEFAULT '0000000' NOT NULL,
			  title varchar(250) NOT NULL,
			  description varchar(250) NOT NULL,
			  diskfile varchar(250) NOT NULL,
			  filename varchar(250) NOT NULL,
...
# more varchar to enum conversions
	...
	$upgrades[] = new SQLUpgrade(
			'0.14-112',
			'more varchar to enum conversions',
			"ALTER TABLE $t_candidate_table CHANGE severity severity INT (2) DEFAULT '50' not null"
		);

	$upgrades[] = new SQLUpgrade(
			'0.14-113',
			'more varchar to enum conversions',
			"ALTER TABLE $t_candidate_table CHANGE reproducibility reproducibility INT (2) DEFAULT '10' not null"
		);
