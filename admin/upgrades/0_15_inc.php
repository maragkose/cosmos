...
<?php
	...
	$upgrades[] = new FunctionUpgrade(
			'0.15-1',
			'Add file type column to candidate file table',
			'upgrade_0_15_1' );

	function upgrade_0_15_1() {
		global $t_candidate_file_table;

		if ( !db_field_exists( 'file_type', $t_candidate_file_table ) ) {
			$query = "ALTER TABLE $t_candidate_file_table ADD file_type VARCHAR(250) NOT NULL AFTER filesize";
	...
	$upgrades[] = new SQLUpgrade(
			'0.15-3',
			'',
			"ALTER TABLE $t_candidate_table CHANGE os_build os_build VARCHAR(32) NOT NULL"
		);

	$upgrades[] = new SQLUpgrade(
			'0.15-4',
			'',
			"ALTER TABLE $t_candidate_table CHANGE build build VARCHAR(32) NOT NULL"
		);

	$upgrades[] = new SQLUpgrade(
			'0.15-5',
			'',
			"ALTER TABLE $t_candidate_table CHANGE votes votes INT(4) NOT NULL"
		);
	...
	$upgrades[] = new SQLUpgrade(
			...
			"CREATE TABLE IF NOT EXISTS $t_candidate_history_table (
			  user_id int(7) unsigned zerofill NOT NULL default '0000000',
			  candidate_id int(7) unsigned zerofill NOT NULL default '0000000',
