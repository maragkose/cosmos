...
<?php
	...
	$upgrades[] = new FunctionUpgrade(
		'escaping-fix-1',
		'Fix double escaped data in mantis_candidate_file_table',
		'upgrade_escaping_fix_1' );

	function upgrade_escaping_fix_1() {
		global $t_candidate_file_table;
		return upgrade_fix_strings( $t_candidate_file_table, 'id',
			array( 'title', 'description', 'filename' ) );
	}

	$upgrades[] = new FunctionUpgrade(
		'escaping-fix-2',
		'Fix double escaped data in mantis_candidate_table',
		'upgrade_escaping_fix_2' );

	function upgrade_escaping_fix_2() {
		global $t_candidate_table;
		return upgrade_fix_strings( $t_candidate_table, 'id',
			array( 'os', 'os_build', 'platform', 'version', 'build', 'summary' ) );
	}

	$upgrades[] = new FunctionUpgrade(
		'escaping-fix-3',
		'Fix double escaped data in mantis_candidate_text_table',
		'upgrade_escaping_fix_3' );

	function upgrade_escaping_fix_3() {
		global $t_candidate_text_table;
		return upgrade_fix_strings( $t_candidate_text_table, 'id',
			array( 'description', 'steps_to_reproduce', 'additional_information' ) );
	}

	$upgrades[] = new FunctionUpgrade(
		'escaping-fix-4',
		'Fix double escaped data in mantis_candidatenote_text_table',
		'upgrade_escaping_fix_4' );

	function upgrade_escaping_fix_4() {
		global $t_candidatenote_text_table;
		return upgrade_fix_strings( $t_candidatenote_text_table, 'id',
			array( 'note' ) );
	}
	...
	$upgrades[] = new FunctionUpgrade(
		'escaping-fix-9',
		'Fix double escaped data in mantis_candidate_history_table',
	...
	function upgrade_escaping_fix_9() {
		...
		if ( db_field_exists( 'id', $t_candidate_history_table ) ) {
			return upgrade_fix_strings( $t_candidate_history_table, 'id',
	...
	$upgrades[] = new SQLUpgrade(
		...
		"DELETE FROM $t_candidate_history_table
