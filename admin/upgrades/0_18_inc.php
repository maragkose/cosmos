...
<?php
	...
	$upgrades[] = new FunctionUpgrade(
		'0.18-vb-1',
		'Add index on candidate_id field in mantis_candidate_file_table.',
		'upgrade_0_18_vb_1' );
	...
	function upgrade_0_18_vb_1() {
		global $t_candidate_file_table;

		if ( !db_key_exists_on_field( $t_candidate_file_table, 'candidate_id', 'MUL' ) ) {
			$query = "ALTER TABLE $t_candidate_file_table ADD INDEX candidate_id ( candidate_id )";
	...
	function emailseverities_fix_1() {
		...
		if ( !db_field_exists( 'email_on_candidatenote_minimum_severity', $t_user_pref_table ) ) {
			$query = "ALTER TABLE $t_user_pref_table ADD email_on_candidatenote_minimum_severity INT(2) DEFAULT '10' NOT NULL
		...
		if ( !db_field_exists( 'email_on_reopened_minimum_severity', $t_user_pref_table ) ) {
			$query = "ALTER TABLE $t_user_pref_table ADD email_on_reopened_minimum_severity INT(2) DEFAULT '10' NOT NULL
						AFTER email_on_candidatenote_minimum_severity";
	...
	$upgrades[] = new SQLUpgrade(
			...
			"CREATE TABLE IF NOT EXISTS mantis_sponsorship_table (
				...
				candidate_id int(7) NOT NULL default '0',
	...
	$upgrades[] = new SQLUpgrade(
			'sponsorship-2',
			'Add sponsorship_total to candidate table',
			"ALTER TABLE $t_candidate_table ADD sponsorship_total INT( 7 ) DEFAULT '0' NOT NULL" );

	$upgrades[] = new SQLUpgrade(
			'sponsorship-3',
			'Add an index on sponsorship_total in candidate table',
			"ALTER TABLE $t_candidate_table ADD INDEX sponsorship_total ( sponsorship_total )" );

	$upgrades[] = new SQLUpgrade(
			'fixed_in_version-1',
			'Add fixed_in_version field to candidate table.',
			"ALTER TABLE $t_candidate_table ADD fixed_in_version VARCHAR( 64 ) NOT NULL AFTER version" );

	$upgrades[] = new SQLUpgrade(
			'fixed_in_version-2',
			'Add index on fixed_in_version field in candidate table.',
			"ALTER TABLE $t_candidate_table ADD INDEX ( fixed_in_version )" );
	...
	# - Add index on source_candidate_id field in mantis_candidate_relationship_table
	# - Add index on destination_candidate_id field in mantis_candidate_relationship_table
	# - For each candidate with the duplicate_id field filled out:
	#      Check if there is already a relationship set between candidate and duplicate candidate
	#      if yes then do nothing and continue with the next candidate
	#      if no then
	#         add the relationship between candidate and duplicate candidate
	#         search in history of candidate the line in which is recorded the duplicate id information
	#         and replace it with the appropriate history record for relationship (same user id, same time-stamp)
	#         add an history record in the duplicate candidate with same user id, same time-stamp with candidate
	...
	function upgrade_0_18_relationship_1() {
		global $t_candidate_relationship_table;

		if ( !db_key_exists_on_field( $t_candidate_relationship_table, 'source_candidate_id', 'MUL' ) ) {
			$query = "ALTER TABLE $t_candidate_relationship_table ADD INDEX ( source_candidate_id )";
			$result = @db_query( $query );
	...
	function upgrade_0_18_relationship_2() {
		...
		if ( !db_key_exists_on_field( $t_candidate_relationship_table, 'destination_candidate_id', 'MUL' ) ) {
			$query = "ALTER TABLE $t_candidate_relationship_table ADD INDEX ( destination_candidate_id )";
			$result = @db_query( $query );
	...
	function upgrade_0_18_relationship_3() {
		global $t_candidate_relationship_table, $t_candidate_table, $t_candidate_history_table;
		...
		for ( $i = 0 ; $i < $t_count ; $i++ ) {
			$t_candidate = db_fetch_array( $result );
			$t_candidate_id = $t_candidate['id'];
			$t_duplicate_candidate_id = $t_candidate['duplicate_id'];

			$query = "SELECT id
						FROM $t_candidate_relationship_table
						WHERE
							(source_candidate_id = '$t_candidate_id' and
							destination_candidate_id = '$t_duplicate_candidate_id') or
							(destination_candidate_id = '$t_candidate_id' and
							source_candidate_id = '$t_duplicate_candidate_id');";
			...
			$query = "INSERT INTO $t_candidate_relationship_table
					( source_candidate_id, destination_candidate_id, relationship_type )
					VALUES
					( '" . $t_candidate_id . "', '" . $t_duplicate_candidate_id . "', '" . BUG_DUPLICATE . "')";
			db_query( $query );

			$query = "SELECT id, user_id, date_modified
						FROM $t_candidate_history_table
						WHERE
							candidate_id = '$t_candidate_id' and
							field_name = 'duplicate_id' and
							new_value = '$t_duplicate_candidate_id';";
			$result2 = db_query( $query );
			$t_count2 = db_num_rows( $result2 );
			...
			$t_history_candidate = db_fetch_array( $result2 );
			$t_history_id = $t_history_candidate['id'];
			$t_history_user_id = $t_history_candidate['user_id'];
			$t_duplicate_candidate_last_update = $t_history_candidate['date_modified'];

			$query = "UPDATE $t_candidate_history_table
				SET field_name = '',
			...
			$query = "INSERT INTO $t_candidate_history_table
				( user_id, candidate_id, date_modified, type, old_value, new_value )
				VALUES
				( '$t_history_user_id', '$t_duplicate_candidate_id', '" . $t_duplicate_candidate_last_update . "', " .
				BUG_ADD_RELATIONSHIP . ", " . BUG_HAS_DUPLICATE . ", '$t_candidate_id' )";
			db_query( $query );
		}
	...
	function upgrade_0_18_relationship_4() {
		global $t_candidate_relationship_table, $t_candidate_table;
		...
		for ( $i = 0 ; $i < $t_count ; $i++ ) {
			...
			$query = "SELECT id
						...
						WHERE
							...
							source_candidate_id = '$t_duplicate_candidate_id' and
							destination_candidate_id = '$t_candidate_id';";
			$result2 = db_query( $query );
			$t_count2 = db_num_rows( $result2 );
			...
			$query = "UPDATE $t_candidate_relationship_table
				SET source_candidate_id = '$t_candidate_id',
				destination_candidate_id = '$t_duplicate_candidate_id'
    ...
    $upgrades[] = new SQLUpgrade(
			'0.18-candidatenote-limit',
			'Add email_candidatenote_limit to user preference table',
			"ALTER TABLE $t_user_pref_table ADD email_candidatenote_limit INT( 2 ) NOT NULL AFTER email_on_new_minimum_severity" );

	$upgrades[] = new SQLUpgrade(
			'0.18-candidatenote-order',
			'Add candidatenote_order to user preference table',
			"ALTER TABLE $t_user_pref_table ADD candidatenote_order VARCHAR( 4 ) NOT NULL DEFAULT '" . config_get_global( 'default_candidatenote_order' ) . "' AFTER redirect_delay" );
	...
	function upgrade_0_19_checkbox_list_multilist_upgrade() {
		...
		$query = "SELECT f.field_id, f.candidate_id, f.value FROM $t_custom_field_string_table f
		...
		for ( $i = 0; $i < $t_count; $i++ ) {
			...
			if ( '' != $t_value ) {
			    ...
			    $t_candidate_id = $t_row['candidate_id'];
			    $query = "UPDATE $t_custom_field_string_table
				      SET value = '|$t_value|'
				      WHERE (field_id = $t_field_id) AND (candidate_id = $t_candidate_id)";
			    db_query( $query );
			}
		}
