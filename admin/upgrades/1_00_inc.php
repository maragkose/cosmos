...
<?php
	...
	$upgrades[] = new SQLUpgrade(
			'note_candidate_id_index',
			'Add index on candidate_id in candidatenotes table',
			"ALTER TABLE $t_candidatenote_table ADD INDEX ( candidate_id )"
		);
	...
	$upgrades[] = new SQLUpgrade(
			...
			'Add index on status in candidate table',
			"ALTER TABLE $t_candidate_table ADD INDEX ( status )"
		);

	$upgrades[] = new SQLUpgrade(
			'candidate_project_index',
			'Add index on project_id in candidate table',
			"ALTER TABLE $t_candidate_table ADD INDEX ( project_id )"
		);

	$upgrades[] = new SQLUpgrade(
			'note_updated_index',
			'Add index on last_modified in candidatenotes table',
			"ALTER TABLE $t_candidatenote_table ADD INDEX ( last_modified )"
		);
	...
	$upgrades[] = new SQLUpgrade(
			'cf_string_candidate_index',
			'Add index on candidate_id in custom_field_string table',
			"ALTER TABLE $t_custom_field_string_table ADD INDEX ( candidate_id )"
		);
	...
	$upgrades[] = new SQLUpgrade(
			'pref_candidatenote_min',
			'change pref email_on_candidatenote_minimum_severity for database compabilility',
			"ALTER TABLE $t_user_pref_table CHANGE email_on_candidatenote_minimum_severity email_on_candidatenote_min_severity INT(2) DEFAULT '10' NOT NULL"
		);
	...
	$upgrades[] = new SQLUpgrade(
			...
			"ALTER TABLE $t_candidate_table CHANGE fixed_in_version fixed_in_version VARCHAR(64) DEFAULT '' NOT NULL"
		);
