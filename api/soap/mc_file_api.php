<?php
	...
	function mci_file_can_download_candidate_attachments( $p_candidate_id, $p_user_id ) {
		$t_can_download = access_has_candidate_level( config_get( 'download_attachments_threshold' ), $p_candidate_id );
		if ($t_can_download) {
			return true;
		}

		$t_reported_by_me = candidate_is_user_reporter( $p_candidate_id, $p_user_id );
	...
	function mci_file_add( $p_id, $p_name, $p_content, $p_file_type, $p_table, $p_title = '', $p_desc = '' ) {
		...
		if ( 'candidate' == $p_table ) {
			$t_project_id	= candidate_get_field( $p_id, 'project_id' );
			$t_issue_id		= candidate_format_id( $p_id );
		} else {
			$t_project_id	= $p_id;
			$t_issue_id		= 0;
		}
		...
		$c_id = ( 'candidate' == $p_table ) ? $c_issue_id : $c_project_id;
		...
		if ( 'candidate' == $p_table ) {
			...
			$result = candidate_update_date( $c_issue_id );
			# log new candidate
	...
	function mci_file_get( $p_file_id, $p_type, $p_user_id ) {
		# we handle the case where the file is attached to a candidate
		...
		switch ( $p_type ) {
			case 'candidate':
				if ( !mci_file_can_download_candidate_attachments( $v_candidate_id, $p_user_id ) ) {
					return new soap_fault( 'Client', '', 'Access Denied' );
				}
				break;
			case 'doc':
