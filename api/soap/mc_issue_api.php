<?php
	...
	function mc_issue_exists( $p_username, $p_password, $p_issue_id ) {
		...
		if ( !candidate_exists( $p_issue_id ) ) {
			return false;
		}

		$t_project_id = candidate_get_field( $p_issue_id, 'project_id' );
	...
	function mc_issue_get( $p_username, $p_password, $p_issue_id ) {
		...
		$t_candidate = get_object_vars( candidate_get( $p_issue_id, true ) );
		$t_issue_data = array();

		$t_issue_data['id'] = $p_issue_id;
 		$t_issue_data['view_state'] = mci_enum_get_array_by_id( $t_candidate['view_state'], 'view_state', $t_lang );
 		$t_issue_data['last_updated'] = timestamp_to_iso8601( $t_candidate['last_updated'] );

		$t_issue_data['project'] = mci_project_as_array_by_id( $t_candidate['project_id'] );
		$t_issue_data['category'] = mci_null_if_empty( $t_candidate['category'] );
 		$t_issue_data['priority'] = mci_enum_get_array_by_id( $t_candidate['priority'], 'priority', $t_lang );
 		$t_issue_data['severity'] = mci_enum_get_array_by_id( $t_candidate['severity'], 'severity', $t_lang );
 		$t_issue_data['status'] = mci_enum_get_array_by_id( $t_candidate['status'], 'status', $t_lang );

 		$t_issue_data['reporter'] = mci_account_get_array_by_id( $t_candidate['reporter_id'] );
		$t_issue_data['summary'] = $t_candidate['summary'];
		$t_issue_data['version'] = mci_null_if_empty( $t_candidate['version'] );
		$t_issue_data['build'] = mci_null_if_empty( $t_candidate['build'] );
		$t_issue_data['platform'] = mci_null_if_empty( $t_candidate['platform'] );
		$t_issue_data['os'] = mci_null_if_empty( $t_candidate['os'] );
		$t_issue_data['os_build'] = mci_null_if_empty( $t_candidate['os_build'] );
 		$t_issue_data['reproducibility'] = mci_enum_get_array_by_id( $t_candidate['reproducibility'], 'reproducibility', $t_lang );
 		$t_issue_data['date_submitted'] = timestamp_to_iso8601( $t_candidate['date_submitted'] );

		$t_issue_data['sponsorship_total'] = $t_candidate['sponsorship_total'];

 		if( !empty( $t_candidate['handler_id'] ) ) {
 			$t_issue_data['handler'] = mci_account_get_array_by_id( $t_candidate['handler_id'] );
 		}
 		$t_issue_data['projection'] = mci_enum_get_array_by_id( $t_candidate['projection'], 'projection', $t_lang );
 		$t_issue_data['eta'] = mci_enum_get_array_by_id( $t_candidate['eta'], 'eta', $t_lang );

 		$t_issue_data['resolution'] = mci_enum_get_array_by_id( $t_candidate['resolution'], 'resolution', $t_lang );
		$t_issue_data['fixed_in_version'] = mci_null_if_empty( $t_candidate['fixed_in_version'] );

		$t_issue_data['description'] = $t_candidate['description'];
		$t_issue_data['steps_to_reproduce'] = mci_null_if_empty( $t_candidate['steps_to_reproduce'] );
		$t_issue_data['additional_information'] = mci_null_if_empty( $t_candidate['additional_information'] );
	...
	function mci_issue_get_attachments( $p_issue_id ) {
		$t_attachment_rows = candidate_get_attachments( $p_issue_id );
		$t_result = array();
		...
		foreach( $t_attachment_rows as $t_attachment_row ) {
			...
			$t_attachment['download_url'] = mci_get_cosmos_path() . 'file_download.php?file_id=' . $t_attachment_row['id'] . '&amp;type=candidate';
	...
	function mci_issue_get_relationships( $p_issue_id, $p_user_id ) {
		...
		foreach( $t_src_relationships as $t_relship_row) {
			if ( access_has_candidate_level( config_get( 'mc_readonly_access_level_threshold' ), $t_relship_row->dest_candidate_id, $p_user_id ) ) {
				...
				$t_relationship['target_id'] = $t_relship_row->dest_candidate_id;
		...
		foreach( $t_dest_relationships as $t_relship_row) {
			if ( access_has_candidate_level( config_get( 'mc_readonly_access_level_threshold' ), $t_relship_row->src_candidate_id, $p_user_id ) ) {
				$t_relationship = array();
				$t_relationship['id'] = $t_relship_row->id;
				$t_reltype = array();
				$t_reltype['id'] = $t_relship_row->type;
				...
				$t_relationship['type'] = $t_reltype;
				$t_relationship['target_id'] = $t_relship_row->src_candidate_id;
				$t_relationships[] = $t_relationship;
			}
		}
	...
	function mci_issue_get_notes( $p_issue_id ) {
		...
		$t_user_candidatenote_order = 'ASC'; // always get the notes in ascending order for consistency to the calling application.

		$t_result = array();
		foreach( candidatenote_get_all_visible_candidatenotes( $p_issue_id, $t_user_access_level, $t_user_candidatenote_order, 0 ) as $t_value ) {
			$t_candidatenote = array();
			$t_candidatenote['id'] = $t_value->id;
			$t_candidatenote['reporter'] = mci_account_get_array_by_id( $t_value->reporter_id );
			$t_candidatenote['date_submitted'] = timestamp_to_iso8601( $t_value->date_submitted );
			$t_candidatenote['last_modified'] = timestamp_to_iso8601( $t_value->last_modified );
			$t_candidatenote['text'] = $t_value->note;
 			$t_candidatenote['view_state'] = mci_enum_get_array_by_id( $t_value->view_state, 'view_state', $t_lang );
			$t_result[] = $t_candidatenote;
		}
	...
	function mc_issue_get_biggest_id( $p_username, $p_password, $p_project_id ) {
		...
		$t_rows = filter_get_candidate_rows( $t_page_number, $t_per_page, $t_page_count, $t_candidate_count, $t_filter, $t_project_id, $t_user_id );
		if ( count( $t_rows ) == 0 ) {
			return 0;
		} else {
			return $t_rows[0]['id'];
	...
	function mc_issue_get_id_from_summary( $p_username, $p_password, $p_summary ) {
		...
		if ( db_num_rows( $result ) == 0 ) {
			...
			while ( ( $row = db_fetch_array( $result ) ) !== false ) {
				...
				$t_project_id = candidate_get_field( $t_issue_id, 'project_id' );
	...
	function mc_issue_add( $p_username, $p_password, $p_issue ) {
		...
		if ( !access_has_project_level( config_get( 'report_candidate_threshold' ), $t_project_id, $t_user_id ) ) {
			return new soap_fault( 'Client', '', "User '$t_user_id' does not have access right to report issues." );
		}

		#if ( !access_has_project_level( config_get( 'report_candidate_threshold' ), $t_project_id ) ||
		#	!access_has_project_level( config_get( 'report_candidate_threshold' ), $t_project_id, $v_reporter ) ) {
		...
		if ( $v_priority == 0 ) {
			$v_priority = config_get( 'default_candidate_priority' );
		}

		if ( $v_severity == 0 ) {
			$v_severity = config_get( 'default_candidate_severity' );
		}

		if ( $v_view_state == 0 ) {
			$v_view_state = config_get( 'default_candidate_view_status' );
		}
		...
		$t_candidate_data->project_id = $t_project_id;
		$t_candidate_data->reporter_id = $t_reporter_id;
		$t_candidate_data->handler_id = $t_handler_id;
		$t_candidate_data->priority = $t_priority_id;
		$t_candidate_data->severity = $t_severity_id;
		$t_candidate_data->reproducibility = $t_reproducibility_id;
		$t_candidate_data->status = $t_status_id;
		$t_candidate_data->resolution = $t_resolution_id;
		$t_candidate_data->projection = $t_projection_id;
		$t_candidate_data->category = $v_category;
		$t_candidate_data->date_submitted = isset( $v_date_submitted ) ? $v_date_submitted : '';
		$t_candidate_data->last_updated = isset( $v_last_updated ) ? $v_last_updated : '';
		$t_candidate_data->eta = $t_eta_id;
		$t_candidate_data->os = isset( $v_os ) ? $v_os : '';
		$t_candidate_data->os_build = isset( $v_os_build ) ? $v_os_build : '';
		$t_candidate_data->platform = isset( $v_platform ) ? $v_platform : '';
		$t_candidate_data->version = isset( $v_version ) ? $v_version : '';
		$t_candidate_data->fixed_in_version = isset( $v_fixed_in_version ) ? $v_fixed_in_version : '';
		$t_candidate_data->build = isset( $v_build ) ? $v_build : '';
		$t_candidate_data->view_state = $t_view_state_id;
		$t_candidate_data->summary = $v_summary;
		$t_candidate_data->sponsorship_total = isset( $v_sponsorship_total ) ? $v_sponsorship_total : 0;

		# omitted:
		# var $candidate_text_id
		# $t_candidate_data->profile_id;

		# extended info
		$t_candidate_data->description = $v_description;
		$t_candidate_data->steps_to_reproduce = isset( $v_steps_to_reproduce ) ? $v_steps_to_reproduce : '';
		$t_candidate_data->additional_information = isset( $v_additional_information ) ? $v_additional_information : '';

		# submit the issue
		$t_issue_id = candidate_create( $t_candidate_data );
		...
		if ( isset( $v_notes ) && is_array( $v_notes ) ) {
			foreach( $v_notes as $t_note ) {
		 		if ( isset( $t_note['view_state'] ) ) {
			 		...
			 		$t_view_state = config_get( 'default_candidatenote_view_status' );
			 	}

				$t_view_state_id = mci_get_enum_id_from_objectref( 'view_state', $t_view_state );
				candidatenote_add( $t_issue_id, $t_note['text'], '0:00', $t_view_state_id == VS_PRIVATE, BUGNOTE, '', $t_user_id, FALSE );
			}
		}

		email_new_candidate( $t_issue_id );
    ...
    function mc_issue_update( $p_username, $p_password, $p_issue_id, $p_issue ) {
        ...
        if ( !access_has_candidate_level( config_get( 'update_candidate_threshold' ), $p_issue_id, $t_user_id ) ) {
            return new soap_fault( 'Client', '', "User '$t_user_id' does not have access right to report issues." );
        }
        ...
        # submit the issue
        $t_is_success = candidate_update($p_issue_id,$t_candidate_data,true,false );
        ...
        if ( isset( $v_notes ) && is_array( $v_notes ) ) {
            foreach( $v_notes as $t_note ) {
				...
				// TODO: consider supporting updating of candidatenotes and detecting the ones that haven't changed.
                $t_view_state_id = mci_get_enum_id_from_objectref( 'view_state', $t_view_state );
                candidatenote_add( $p_issue_id, $t_note['text'], '0:00', $t_view_state_id == VS_PRIVATE, BUGNOTE, '', $t_user_id, FALSE );
            }
        }
	...
	function mc_issue_delete( $p_username, $p_password, $p_issue_id ) {
		...
		return candidate_delete( $p_issue_id );
	}
	 ...
	 */
 	function mc_issue_note_add( $p_username, $p_password, $p_issue_id, $p_note ) {
		...
		$t_project_id = candidate_get_field( $p_issue_id, 'project_id' );
 		...
 		if ( !access_has_candidate_level( config_get( 'add_candidatenote_threshold' ), $p_issue_id, $t_user_id ) ) {
 			return new soap_fault( 'Client', '', "User '$t_user_id' does not have access right to add notes to this issue." );
 		}

 		if ( candidate_is_readonly( $p_issue_id ) ) {
 		...
 		if ( isset( $p_note['view_state'] ) ) {
	 		...
	 		$t_view_state = array( 'id' => config_get( 'default_candidate_view_status' ) );
	 	}

		$t_view_state_id = mci_get_enum_id_from_objectref( 'view_state', $t_view_state );
		return candidatenote_add( $p_issue_id, $p_note['text'], '0:00', $t_view_state_id == VS_PRIVATE, BUGNOTE, '', $t_user_id );
 	}
	 ...
	 */
 	function mc_issue_note_delete( $p_username, $p_password, $p_issue_note_id ) {
		...
		if ( !candidatenote_exists( $p_issue_note_id ) ) {
			return new soap_fault( 'Server', '', "Issue note '$p_issue_note_id' does not exist." );
		}

		$t_issue_id = candidatenote_get_field( $p_issue_note_id, 'candidate_id' );
		$t_project_id = candidate_get_field( $t_issue_id, 'project_id' );
		if ( !mci_has_readwrite_access( $t_user_id, $t_project_id ) ) {
			return new soap_fault( 'Client', '', 'Access Denied' );
		}

		return candidatenote_delete( $p_issue_note_id );
 	}
	 ...
	 */
 	function mc_issue_relationship_add( $p_username, $p_password, $p_issue_id, $p_relationship ) {
		...
		if ( !candidate_exists( $t_dest_issue_id ) ) {
		...
		if ( !access_has_candidate_level( VIEWER, $t_dest_issue_id, $t_user_id ) ) {
 		...
 		if ( $t_old_id_relationship == 0 ) {
 			...
 			$t_relationship_id = db_insert_id( config_get( 'cosmos_candidate_relationship_table' ) );
			...
			candidate_update_date( $p_issue_id );
	 ...
	 */
 	function mc_issue_relationship_delete( $p_username, $p_password, $p_issue_id, $p_relationship_id ) {
		...
		$t_dest_issue_id = relationship_get_linked_candidate_id( $p_relationship_id, $p_issue_id );
		...
		$t_candidate_relationship_data = relationship_get( $p_relationship_id );
		...
		if ($p_issue_id == $t_candidate_relationship_data->src_candidate_id) {
		...
		history_log_event_special( $p_issue_id, BUG_DEL_RELATIONSHIP, $t_candidate_rel_type, $t_dest_issue_id );
		email_relationship_deleted( $p_issue_id, $t_dest_issue_id, $t_candidate_rel_type );

		if ( candidate_exists( $t_dest_issue_id )) {
			# send email and update the history for the dest issue
			history_log_event_special( $t_dest_issue_id, BUG_DEL_RELATIONSHIP, $t_dest_candidate_rel_type, $p_issue_id );
			email_relationship_deleted( $t_dest_issue_id, $p_issue_id, $t_dest_candidate_rel_type );
		}
