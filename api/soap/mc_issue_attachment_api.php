<?php
	 ...
	 */
 	function mc_issue_attachment_get( $p_username, $p_password, $p_issue_attachment_id ) {
		...
		return mci_file_get( $p_issue_attachment_id, 'candidate', $t_user_id );
 	}
	 ...
	 */
 	function mc_issue_attachment_add( $p_username, $p_password, $p_issue_id, $p_name, $p_file_type, $p_content ) {
	 	...
	 	if ( !file_allow_candidate_upload( $p_issue_id, $t_user_id ) ) {
 			return new soap_fault( 'Client', '', 'Access Denied' );	 	
	 	}
		if ( !access_has_candidate_level( config_get( 'upload_candidate_file_threshold' ), $p_issue_id, $t_user_id ) ) {
 			return new soap_fault( 'Client', '', 'Access Denied' );
		}
		return mci_file_add( $p_issue_id, $p_name, $p_content, $p_file_type, 'candidate' );
 	}
	 ...
	 */
 	function mc_issue_attachment_delete( $p_username, $p_password, $p_issue_attachment_id ) {
		...
		$t_candidate_id = file_get_field( $p_issue_attachment_id, 'candidate_id' );
		if ( !access_has_candidate_level( config_get( 'update_candidate_threshold' ), $t_candidate_id, $t_user_id ) ) {
			return new soap_fault( 'Client', '', 'Access Denied' );
		}
		return file_delete( $p_issue_attachment_id, 'candidate' );
 	}
?>
