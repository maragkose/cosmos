<?php
	...
	function mc_project_get_issues( $p_username, $p_password, $p_project_id , $p_page_number, $p_per_page ) {
        ...
        $t_rows = filter_get_candidate_rows( $p_page_number, $p_per_page, $t_page_count, $t_candidate_count, null, $p_project_id );
        $t_result = array();
        ...
        foreach( $t_rows as $t_issue_data ) {
            ...
            $t_issue['description'] = candidate_get_text_field( $t_id, 'description' );
            $t_issue['steps_to_reproduce'] = mci_null_if_empty( candidate_get_text_field( $t_id, 'steps_to_reproduce' ) );
            $t_issue['additional_information'] = mci_null_if_empty( candidate_get_text_field( $t_id, 'additional_information' ) );
	...
	function mc_project_get_attachments( $p_username, $p_password, $p_project_id ) {
		...
		for ($i=0;$i<$num_files;$i++) {
			...
			$t_attachment['download_url'] = mci_get_cosmos_path() . 'file_download.php?file_id=' . $v_id . '&amp;type=doc';
