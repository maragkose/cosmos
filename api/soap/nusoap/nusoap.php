...
$GLOBALS['_transient']['static']['nusoap_base']->globalDecandidateLevel = 9;
...
class nusoap_base {
	...
	var $error_str = '';
    /**
     * Current debug string (manipulated by debug/appendDecandidate/clearDecandidate/getDecandidate/getDecandidateAsXMLComment)
	 *
	 * @var string
	 * @access private
	 */
	...
	*/
	function nusoap_base() {
		$this->debugLevel = $GLOBALS['_transient']['static']['nusoap_base']->globalDecandidateLevel;
	}

	/**
	* gets the global debug level, which applies to future instances
	*
	* @return	integer	Decandidate level 0-9, where 0 turns off
	* @access	public
	*/
	function getGlobalDecandidateLevel() {
		return $GLOBALS['_transient']['static']['nusoap_base']->globalDecandidateLevel;
	}

	/**
	* sets the global debug level, which applies to future instances
	*
	* @param	int	$level	Decandidate level 0-9, where 0 turns off
	* @access	public
	*/
	function setGlobalDecandidateLevel($level) {
		$GLOBALS['_transient']['static']['nusoap_base']->globalDecandidateLevel = $level;
	}

	/**
	* gets the debug level for this instance
	*
	* @return	int	Decandidate level 0-9, where 0 turns off
	* @access	public
	*/
	function getDecandidateLevel() {
	...
	* @access	public
	*/
	function setDecandidateLevel($level) {
	...
	*/
	function debug($string){
		if ($this->debugLevel > 0) {
			$this->appendDecandidate($this->getmicrotime().' '.get_class($this).": $string\n");
		}
	}
	...
	* @access   public
	*/
	function appendDecandidate($string){
		if ($this->debugLevel > 0) {
	...
	* @access   public
	*/
	function &getDecandidateAsXMLComment() {
		// it would be nice to use a memory stream here to use
		// memory more efficiently
	...
	*/
	function serialize_val($val,$name=false,$type=false,$name_ns=false,$type_ns=false,$attributes=false,$use='encoded',$soapval=false) {
		...
		$this->appendDecandidate('value=' . $this->varDump($val));
		$this->appendDecandidate('attributes=' . $this->varDump($attributes));
		
    	if (is_object($val) && get_class($val) == 'soapval' && (! $soapval)) {
    		$this->debug("serialize_val: serialize soapval");
        	$xml = $val->serialize($use);
			$this->appendDecandidate($val->getDecandidate());
			$val->clearDecandidate();
    ...
    */
	...
	$this->debug("headers:");
	$this->appendDecandidate($this->varDump($headers));
	$this->debug("namespaces:");
	$this->appendDecandidate($this->varDump($namespaces));
...
class nusoap_xmlschema extends nusoap_base  {
	...
	function schemaStartElement($parser, $name, $attrs) {
		...
		switch($name){
			...
			case 'attribute':	// complexType attribute
            	...
            	$this->appendDecandidate($this->varDump($attrs));
				if (!isset($attrs['form'])) {
	...
	*/
	function addComplexType($name,$typeClass='complexType',$phpType='array',$compositor='',$restrictionBase='',$elements=array(),$attrs=array(),$arrayType=''){
		...
		$this->appendDecandidate($this->varDump($this->complexTypes[$name]));
	}
	...
	*/
	function addSimpleType($name, $restrictionBase='', $typeClass='simpleType', $phpType='scalar', $enumeration=array()) {
		...
		$this->appendDecandidate($this->varDump($this->simpleTypes[$name]));
	}
	...
	*/
	function addElement($attrs) {
		...
		$this->appendDecandidate($this->varDump($this->elements[ $attrs['name'] ]));
	}
}
...
class soap_transport_http extends nusoap_base {
	...
	*/
	function soap_transport_http($url, $curl_options = NULL, $use_curl = false){
		...
		$this->appendDecandidate($this->varDump($curl_options));
		$this->setURL($url);
	...
	*/
	function setCurlOption($option, $value) {
		...
		$this->appendDecandidate($this->varDump($value));
...
//			$this->setError('cURL Extension, or OpenSSL extension w/ PHP version >= 4.3 is required for HTTPS');
		...
		if (ini_get('safe_mode') || ini_get('open_basedir')) {
			...
			$this->appendDecandidate($this->varDump(ini_get('safe_mode')));
			$this->debug('open_basedir = ');
			$this->appendDecandidate($this->varDump(ini_get('open_basedir')));
		} else {
...
//		}
	...
	*/
	function setCredentials($username, $password, $authtype = 'basic', $digestRequest = array(), $certRequest = array()) {
		...
		$this->appendDecandidate($this->varDump($digestRequest));
		$this->debug("certRequest=");
		$this->appendDecandidate($this->varDump($certRequest));
		// cf. RFC 2617
		if ($authtype == 'basic') {
...
//		}
		...
		if ($cErr != '') {
        	...
        	// TODO: there is a PHP candidate that can cause this to SEGV for CURLINFO_CONTENT_TYPE
...
class nusoap_server extends nusoap_base {
	...
	*/
	function nusoap_server($wsdl=false){
		...
		if (isset($_SERVER)) {
			...
			$this->appendDecandidate($this->varDump($_SERVER));
		} elseif (isset($HTTP_SERVER_VARS)) {
			$this->debug("HTTP_SERVER_VARS is defined:");
			$this->appendDecandidate($this->varDump($HTTP_SERVER_VARS));
		} else {
		...
		if($wsdl){
			...
			$this->appendDecandidate($this->wsdl->getDecandidate());
			$this->wsdl->clearDecandidate();
		...
		if (ereg('wsdl', $qs) ){
			...
			if($this->externalWSDLURL){
              if (strpos($this->externalWSDLURL,"://")!==false) { // assume URL
				...
				if ($this->debug_flag) {
					$this->debug('wsdl:');
					$this->appendDecandidate($this->varDump($this->wsdl));
					print $this->getDecandidateAsXMLComment();
				}
			} else {
	...
	function invoke_method() {
		...
		if ($this->wsdl) {
			if ($this->opData = $this->wsdl->getOperationData($this->methodname)) {
				...
				$this->appendDecandidate('opData=' . $this->varDump($this->opData));
		...
		$this->appendDecandidate($this->varDump($this->methodparams));
        ...
        $this->appendDecandidate($this->varDump($this->methodreturn));
	...
	*/
	function serialize_return() {
		...
		$this->appendDecandidate($this->varDump($return_val));
	...
	*/
	function send_response() {
		...
		if(isset($this->debug_flag) && $this->debug_flag){
        	$payload .= $this->getDecandidateAsXMLComment();
        }
	...
	*/
	function fault($faultcode,$faultstring,$faultactor='',$faultdetail=''){
		if ($faultdetail == '' && $this->debug_flag) {
			$faultdetail = $this->getDecandidate();
		}
...
class wsdl extends nusoap_base {
    ...
    function parseWSDL($wsdl = '') {
        ...
        if (isset($wsdl_props['scheme']) && ($wsdl_props['scheme'] == 'http' || $wsdl_props['scheme'] == 'https')) {
			...
			$this->appendDecandidate($tr->getDecandidate());
			// catch errors
			if($err = $tr->getError() ){
    ...
    function start_element($parser, $name, $attrs)
    {
        if ($this->status == 'schema') {
            ...
            $this->appendDecandidate($this->currentSchema->getDecandidate());
            $this->currentSchema->clearDecandidate();
	...
	function getTypeDef($type, $ns) {
		...
		if (isset($this->schemas[$ns])) {
			...
			for ($i = 0; $i < count($this->schemas[$ns]); $i++) {
				...
				//$this->appendDecandidate($xs->getDecandidate());
				//$xs->clearDecandidate();
				if ($t) {
					if (!isset($t['phpType'])) {
						// get info for type to tack onto the element
	...
	*/
	...
	function parametersMatchWrapped($type, &$parameters) {
		...
		$this->appendDecandidate($this->varDump($parameters));
		...
		$this->appendDecandidate($this->varDump($typeDef));
		if (substr($uqType, -1) == '^') {
	...
	function serializeRPCParameters($operation, $direction, $parameters, $bindingType = 'soap') {
		...
		$this->appendDecandidate('parameters=' . $this->varDump($parameters));
		...
		$this->appendDecandidate($this->varDump($opData));
	...
	function serializeType($name, $type, $value, $use='encoded', $encodingStyle=false, $unqualified=false)
	{
		...
		$this->appendDecandidate("value=" . $this->varDump($value));
		if($use == 'encoded' && $encodingStyle) {
		...
		if(!$typeDef = $this->getTypeDef($uqType, $ns)){
			...
			$this->appendDecandidate('typeDef=' . $this->varDump($typeDef));
			if (substr($uqType, -1) == '^') {
				$uqType = substr($uqType, 0, -1);
			}
		}
	...
	*/
	function buildVal($pos){
		...
		if($this->message[$pos]['children'] != ''){
			...
			$this->appendDecandidate($this->varDump($ret));
			return $ret;
		} else {
...
class nusoap_client extends nusoap_base  {
	...
	*/
	function nusoap_client($endpoint,$wsdl = false,$proxyhost = false,$proxyport = false,$proxyusername = false, $proxypassword = false, $timeout = 0, $response_timeout = 30){
		...
		$this->debug("ctor wsdl=$wsdl timeout=$timeout response_timeout=$response_timeout");
		$this->appendDecandidate('endpoint=' . $this->varDump($endpoint));
	...
	*/
	function call($operation,$params=array(),$namespace='http://tempuri.org',$soapAction='',$headers=false,$rpcParams=null,$style='rpc',$use='encoded'){
		...
		$this->appendDecandidate('params=' . $this->varDump($params));
		$this->appendDecandidate('headers=' . $this->varDump($headers));
		if ($headers) {
		...
		if($this->endpointType == 'wsdl' && $opData = $this->getOperationData($operation)){
			...
			$this->appendDecandidate('opData=' . $this->varDump($opData));
		...
		if($errstr = $this->getError()){
			...
			$this->debug('sent message successfully and got a(n) '.gettype($return));
           	$this->appendDecandidate('return=' . $this->varDump($return));
			...
			if(is_array($return) && isset($return['faultcode'])){
				...
				if(is_array($return)){
					...
					$this->appendDecandidate($this->varDump($return));
           			return $return;
	...
	function send($msg, $soapaction = '', $timeout=0, $response_timeout=30) {
		...
		switch(true){
			...
			case ereg('^http',$this->endpoint):
				...
				$this->request = $http->outgoing_payload;
				$this->response = $http->incoming_payload;
				$this->appendDecandidate($http->getDecandidate());
				$this->UpdateCookies($http->incoming_cookies);

				// save transport object if using persistent connections
				if ($this->persistentConnection) {
					$http->clearDecandidate();
	...
	*/
    function parseResponse($headers, $data) {
		...
		$this->appendDecandidate($parser->getDecandidate());
		// if parse errors
	...
	*/
	function _getProxyClassCode($r) {
		...
		$this->appendDecandidate("wsdl=" . $this->varDump($this->wsdl));
