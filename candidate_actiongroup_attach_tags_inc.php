...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------------------------------------------
	# $Id: candidate_actiongroup_attach_tags_inc.php,v 1.1.2.2 2007-10-13 22:32:31 giallu Exp $
	# --------------------------------------------------------
	 ...
	 * Gets called for every candidate, but performs the real tag validation only
	 ...
	 * @return boolean True
	 */
	function action_attach_tags_validate( $p_candidate_id ) {
	...
	/**
	 * Attaches all the tags to each candidate in the group action.
	 * @param integer Bug ID
	 * @return boolean True if all tags attach properly
	 */
	function action_attach_tags_process( $p_candidate_id ) {
		...
		foreach( $g_action_attach_tags_attach as $t_tag_row ) {
			if ( ! tag_candidate_is_attached( $t_tag_row['id'], $p_candidate_id ) ) {
				tag_candidate_attach( $t_tag_row['id'], $p_candidate_id, $t_user_id );
			}
		}
