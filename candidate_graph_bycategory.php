...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------------------------------------------
	# $Id: candidate_graph_bycategory.php,v 1.2.2.1 2007-10-13 22:32:40 giallu Exp $
	# --------------------------------------------------------
?>
<?php
	...
	$t_filter['sort'] = '';
	$rows = filter_get_candidate_rows( $f_page_number, $t_per_page, $t_page_count, $t_candidate_count, $t_filter, null, null, true );
	if ( count($rows) == 0 ) {
		// no data to graph
		exit();
	}
	...
	$t_candidate_hist_table			= config_get( 'cosmos_candidate_history_table' );
    ...
    $t_candidate = array();
    $t_candidate_cat = array(); // save categoties or candidates to look up resolved ones.
    $t_category = array();
	...
	foreach ($rows as $t_row) {
        ...
        $t_candidate[] = $t_row['id'];
        $t_candidate_cat[$t_row['id']] = $t_cat;
	}

    // get the history for these candidates over the interval required to offset the data
    // type = 0 and field=status are status changes
    // type = 1 are new candidates
    $t_select = 'SELECT candidate_id, type, field_name, old_value, new_value, date_modified FROM '.$t_candidate_hist_table.
        ' WHERE candidate_id in ('.implode(',', $t_candidate).') and '.
	        ...
	        switch ($row['type']) {
    	        case 0: // updated candidate
    	            if ($row['field_name'] == 'category') {
                        ...
                        // change the category associated with the candidate to match in case the candidate was
                        //  created during the scan
                        $t_candidate_cat[$row['candidate_id']] = $t_cat; 
                        ...
                        if ( access_compare_level( $row['new_value'], $t_resolved ) && 
                            ...
                            $t_cat = $t_candidate_cat[$row['candidate_id']];
            	            if ($t_cat == '')
            	                $t_cat = 'none';
            	            if (in_array($t_cat, $t_category)) {
                                $t_data[$t_ptr][$t_cat] ++;
...
//  These arise when candidates are opened and closed within the data intervals
