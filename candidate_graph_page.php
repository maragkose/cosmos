...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------------------------------------------
	# $Id: candidate_graph_page.php,v 1.2.2.1 2007-10-13 22:32:42 giallu Exp $
	# --------------------------------------------------------
?>
<?php
...
?>
    <form name="graph_form" method="post" action="candidate_graph_page.php">
...
<?php
    ...
    if ( ( 0 != $f_type ) && ( $f_interval > 0 ) && ( gpc_get( 'show', '' ) != '') ) { 
        ...
        if ( ($t_body == 1 ) || ($t_body == 3) ) {
            if ( $f_show_as_table ) {
                echo '<br /><IFRAME SRC="candidate_graph_bystatus.php?width='.$t_width.'&interval=' . $f_interval . 
                    '&start_date=' . $f_start . '&end_date=' . $f_end .
                    '&summary=' . $t_summary . '&show_table=1" width="100%" height="80%" frameborder="0"' .
                    ' marginwidth="0" marginheight="0"></IFRAME>';
            } else {
                echo '<br /><img src="candidate_graph_bystatus.php?width=600&interval=' . $f_interval . 
                    '&start_date=' . $f_start . '&end_date=' . $f_end .
        ...
        if ( ($t_body == 2 ) || ($t_body == 3) ) {
            if ( $f_show_as_table ) {
                echo '<br /><IFRAME SRC="candidate_graph_bycategory.php?width='.$t_width.'&interval=' . $f_interval . 
                    '&start_date=' . $f_start . '&end_date=' . $f_end .
                    '&summary=' . $t_summary . '&show_table=1" width="100%" height="80%" frameborder="0"' .
                    ' marginwidth="0" marginheight="0"></IFRAME>';
            } else {
                echo '<br /><img src="candidate_graph_bycategory.php?width=600&interval=' . $f_interval . 
                    '&start_date=' . $f_start . '&end_date=' . $f_end .
                    '&summary=' . $t_summary . '&show_table=0" alt="Bug Graph"';
            }
        }
