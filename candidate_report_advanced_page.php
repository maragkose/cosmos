...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------------------------------------------
	# $Id: candidate_report_advanced_page.php,v 1.1.1.1 2009/01/08 07:30:19 chirag Exp $
	# --------------------------------------------------------
	...
	if ( ( ALL_PROJECTS == helper_get_current_project() ) && ( 0 == $f_master_candidate_id ) ) {
		print_header_redirect( 'login_select_proj_page.php?ref=candidate_report_advanced_page.php' );
	}

	if ( SIMPLE_ONLY == config_get( 'show_report' ) ) {
		print_header_redirect ( 'candidate_report_page.php' .
	...
	if( $f_master_candidate_id > 0 ) {
		...
		$f_build				= $t_candidate->build;
		$f_platform				= $t_candidate->platform;
		$f_os					= $t_candidate->os;
		$f_os_build				= $t_candidate->os_build;
		$f_product_version		= $t_candidate->version;
		$f_target_version		= $t_candidate->target_version;
		$f_profile_id			= 0;
		$f_handler_id			= $t_candidate->handler_id;
		...
		$f_steps_to_reproduce	= $t_candidate->steps_to_reproduce;
		...
		$f_steps_to_reproduce	= gpc_get_string( 'steps_to_reproduce', config_get( 'default_candidate_steps_to_reproduce' ) );
	...
	<td class="right">
		<?php
			if ( BOTH == config_get( 'show_report' ) ) {
				print_bracket_link( 'candidate_report_page.php' .
					( $f_master_candidate_id > 0 ? '?m_id=' . $f_master_candidate_id : '' ), lang_get( 'simple_report_link' ) );
			}
		?>
	</td>
</tr>
...
<?php if ( access_has_project_level( config_get( 'update_candidate_assign_threshold' ) ) ) { ?>
<!-- spacer -->
...
<!-- Target Version (if permissions allow) MANOS removed-->
...
<tr <?php echo helper_alternate_class() ?>>
	<td class="category">
		<span class="required">*</span><?php echo lang_get( 'description1' ) ?> <?php print_documentation_link( 'description' ) ?>
	</td>
	<td>
		<textarea <?php echo helper_get_tab_index() ?> name="description" cols="80" rows="2"><?php echo $f_description ?></textarea>
	</td>
</tr>


<!-- Steps to Reproduce MANOS removed-->
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td>
		...
		<?php
				...
				projax_autocomplete( 'os_get_with_prefix', 'os', array( 'value' => $f_os, 'size' => '64', 'maxlength' => '255', 'tabindex' => helper_get_tab_index_value() ) );
			}
		?>
	</td>
</tr>
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td>
		...
		<?php
				...
				projax_autocomplete( 'os_build_get_with_prefix', 'os_build', array( 'value' => $f_os_build, 'size' => '50', 'maxlength' => '100', 'tabindex' => helper_get_tab_index_value() ) );
			}
		?>
	</td>
</tr>
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td>
		...
		<!--<input class="button-small" <?php echo helper_get_tab_index() ?> name="file" type="file" size="60" />-->
		<input class="button-small"  name="file" type="file" size="60" />
	</td>
</tr>
<?php } ?>
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td>
<?php
//Modifed By Chirag A to make it private always
	//if ( access_has_project_level( config_get( 'set_view_status_threshold' ) ) ) {
?>
<input disabled <?php echo helper_get_tab_index() ?> type="radio" name="view_state" value="<?php echo VS_PRIVATE ?>" <?php check_checked( $f_view_state, VS_PRIVATE ) ?> /> <?php echo lang_get( 'private' ) ?>
	</td>
</tr>
