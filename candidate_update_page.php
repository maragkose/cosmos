...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------------------------------------------
	# $Id: candidate_update_page.php,v 1.100.2.1 2007-10-13 22:32:58 giallu Exp $
	# --------------------------------------------------------
	...
	if ( ADVANCED_ONLY == config_get( 'show_update' ) ) {
		print_header_redirect ( 'candidate_update_advanced_page.php?candidate_id=' . $f_candidate_id );
	}
...
<tr>
	<td class="form-title" colspan="3">
		...
		<?php echo lang_get( 'updating_candidate_simple_title' ) ?>
	</td>
	<td class="right" colspan="3">
<?php
	print_bracket_link( string_get_candidate_view_url( $f_candidate_id ), lang_get( 'back_to_candidate_link' ) );

	if ( BOTH == config_get( 'show_update' ) ) {
		print_bracket_link( 'candidate_update_advanced_page.php?candidate_id=' . $f_candidate_id, lang_get( 'update_advanced_link' ) );
	}
?>
	</td>
</tr>
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td bgcolor="<?php echo get_status_color( $t_candidate->status ) ?>">
		<?php print_status_option_list( 'status', $t_candidate->status,
	...
	<?php
		...
		if ( OFF == config_get( 'enable_relationship' ) ) {
			...
			echo '<td>';
			echo '<input ', helper_get_tab_index(), ' type="text" name="duplicate_id" value="', $t_candidate->duplicate_id, '" maxlength="7" />&nbsp;';
			echo '</td>';
		} else {
			# spacer
...
?>
	...
	<td>
		<select <?php echo helper_get_tab_index() ?> name="version">
			<?php print_version_option_list( $t_candidate->version, $t_candidate->project_id, VERSION_RELEASED ) ?>
		</select>
	</td>
</tr>
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td colspan="5">
		<select <?php echo helper_get_tab_index() ?> name="fixed_in_version">
			<?php print_version_option_list( $t_candidate->fixed_in_version, $t_candidate->project_id, VERSION_ALL ) ?>
		</select>
	</td>
</tr>
*/ ?>
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td colspan="5">
		<input <?php echo helper_get_tab_index() ?> type="text" name="summary" size="105" maxlength="128" value="<?php echo $t_candidate->summary ?>" />
	</td>
</tr>
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td colspan="5">
		<textarea <?php echo helper_get_tab_index() ?> cols="80" rows="10" name="description"><?php echo $t_candidate->description ?></textarea>
	</td>
</tr>
...
<tr <?php echo helper_alternate_class() ?>>
	...
	<td colspan="5">
		<textarea <?php echo helper_get_tab_index() ?> cols="80" rows="10" name="additional_information"><?php echo $t_candidate->additional_information ?></textarea>
	</td>
</tr>
...
<?php
	...
	foreach( $t_related_custom_field_ids as $t_id ) {
		...
		if( ( ( $t_def['display_update'] && !$t_def['advanced'] ) || $t_def['require_update']) && custom_field_has_write_access( $t_id, $f_candidate_id ) ) {
