...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------------------------------------------
	# $Id: candidate_view_page.php,v 1.89.2.1 2007-10-13 22:33:03 giallu Exp $
	# --------------------------------------------------------
?>
<?php
...
<?php
	...
	if ( ADVANCED_ONLY == config_get( 'show_view' ) ) {
		print_header_redirect ( 'candidate_view_advanced_page.php?candidate_id=' . $f_candidate_id );
	}
...
<tr>
	...
	<!-- Title -->
	<td class="form-title" colspan="<?php echo $t_candidateslist ? '3' : '4' ?>">
		<?php echo lang_get( 'viewing_candidate_simple_details_title' ) ?>

		<!-- Jump to Bugnotes -->
		<span class="small"><?php print_bracket_link( "#candidatenotes", lang_get( 'jump_to_candidatenotes' ) ) ?></span>
	...
	?>
		<span class="small">
			<?php print_bracket_link( 'candidate_reminder_page.php?candidate_id='.$f_candidate_id, lang_get( 'candidate_reminder' ) ) ?>
		</span>
	<?php
		}
	...
	<td class="center"><span class="small">
		<?php
			...
			if( false !== $t_index ) {
				if( isset( $t_candidateslist[$t_index-1] ) ) print_bracket_link( 'candidate_view_page.php?candidate_id='.$t_candidateslist[$t_index-1], '&lt;&lt;' );
				if( isset( $t_candidateslist[$t_index+1] ) ) print_bracket_link( 'candidate_view_page.php?candidate_id='.$t_candidateslist[$t_index+1], '&gt;&gt;' );
			}
		?>
	</span></td>
	...
	<?php if ( BOTH == config_get( 'show_view' ) ) { ?>
		<span class="small"><?php print_bracket_link( 'candidate_view_advanced_page.php?candidate_id=' . $f_candidate_id, lang_get( 'view_advanced_link' ) )?></span>
	<?php }?>

	<?php if ( $t_can_view_history ) { ?>
		<!-- History -->
		<span class="small"><?php print_bracket_link( 'candidate_view_page.php?candidate_id=' . $f_candidate_id . '&amp;history=1#history', lang_get( 'candidate_history' ) ) ?></span>
	<?php } ?>

		<!-- Print Bug -->
		<span class="small"><?php print_bracket_link( 'print_candidate_page.php?candidate_id=' . $f_candidate_id, lang_get( 'print' ) ) ?></span>
...
<?php
	...
	foreach( $t_related_custom_field_ids as $t_id ) {
		...
		if( !$t_def['advanced'] && custom_field_has_read_access( $t_id, $f_candidate_id ) ) {
...
<?php
	...
	if ( config_get('time_tracking_enabled') &&
		access_has_candidate_level( config_get( 'time_tracking_reporting_threshold' ), $f_candidate_id ) ) {
		include( $t_cosmos_dir . 'candidatenote_stats_inc.php' );
	}
	# History
	if ( $f_history ) {
