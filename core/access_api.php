...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	#  if the user's access is equal to or higher, false otherwise.
	#
	# This function looks up the candidate's project and performs an access check
	#  against that project
	function access_has_candidate_level( $p_access_level, $p_candidate_id, $p_user_id = null ) {
		# Deal with not logged in silently in this case
		# @@@ we may be able to remove this and just error
		#     and once we default to anon login, we can remove it for sure
		if ( !auth_is_user_authenticated() ) {
		...
		if ( ( ON === $t_limit_reporters ) &&
		     ( !candidate_is_user_reporter( $p_candidate_id, $p_user_id ) ) &&
		     ( !access_has_project_level( REPORTER + 1, $t_project_id, $p_user_id ) ) ) {
		  return false;
		}

		# If the candidate is private and the user is not the reporter, then the
		#  the user must also have higher access than private_candidate_threshold
		if ( VS_PRIVATE == candidate_get_field( $p_candidate_id, 'view_state' ) &&
			 !candidate_is_user_reporter( $p_candidate_id, $p_user_id ) ) {
			$p_access_level = max( $p_access_level, config_get( 'private_candidate_threshold' ) );
		}
	...
	# --------------------
	# Check if the user has the specified access level for the given candidate
	#  and deny access to the page if not
	function access_ensure_candidate_level( $p_access_level, $p_candidate_id, $p_user_id=null ) {
		if ( !access_has_candidate_level( $p_access_level, $p_candidate_id, $p_user_id ) ) {
			access_denied();
		}
 	}
	...
	#  if the user's access is equal to or higher, false otherwise.
	#
	# This function looks up the candidatenote's candidate and performs an access check
	#  against that candidate
	function access_has_candidatenote_level( $p_access_level, $p_candidatenote_id, $p_user_id=null ) {
		if ( null===$p_user_id ) {
		    $p_user_id = auth_get_current_user_id();
		}
		...
		# If the candidate is private and the user is not the reporter, then the
		#  the user must also have higher access than private_candidate_threshold
		if ( VS_PRIVATE == candidatenote_get_field( $p_candidatenote_id, 'view_state' ) &&
			 !candidatenote_is_user_reporter( $p_candidatenote_id, $p_user_id ) ) {
			$p_access_level = max( $p_access_level, config_get( 'private_candidatenote_threshold' ) );
		}

		$t_candidate_id = candidatenote_get_field( $p_candidatenote_id, 'candidate_id' );

		return access_has_candidate_level( $p_access_level, $t_candidate_id, $p_user_id );
	}

	# --------------------
	# Check if the user has the specified access level for the given candidatenote
	#  and deny access to the page if not
	function access_ensure_candidatenote_level( $p_access_level, $p_candidatenote_id, $p_user_id=null ) {
		if ( !access_has_candidatenote_level( $p_access_level, $p_candidatenote_id, $p_user_id ) ) {
			access_denied();
		}
 	}

	# --------------------
	# Check if the current user can close the specified candidate
	function access_can_close_candidate ( $p_candidate_id, $p_user_id=null ) {
		if ( null===$p_user_id ) {
		    $p_user_id = auth_get_current_user_id();
		}
		...
		return access_has_candidate_level( access_get_status_threshold( CLOSED ), $p_candidate_id, $p_user_id );
	}

	# --------------------
	# Make sure that the current user can close the specified candidate
	# See access_can_close_candidate() for details.
	function access_ensure_can_close_candidate( $p_candidate_id, $p_user_id=null ) {
		if ( !access_can_close_candidate( $p_candidate_id, $p_user_id ) ) {
			access_denied();
		}
	}

	# --------------------
	# Check if the current user can reopen the specified candidate
	function access_can_reopen_candidate ( $p_candidate_id, $p_user_id=null ) {
		if ( $p_user_id === null ) {
		    $p_user_id = auth_get_current_user_id();
		}
		...
		return access_has_candidate_level( config_get( 'reopen_candidate_threshold' ), $p_candidate_id, $p_user_id );
	}

	# --------------------
	# Make sure that the current user can reopen the specified candidate
	# See access_can_reopen_candidate() for details.
	function access_ensure_can_reopen_candidate( $p_candidate_id, $p_user_id=null ) {
		if ( !access_can_reopen_candidate( $p_candidate_id, $p_user_id ) ) {
			access_denied();
		}
	}
	...
	#  generic update_candidate_status_threshold
	function access_get_status_threshold( $p_status, $p_project_id = ALL_PROJECTS ) {
		...
		if ( isset( $t_thresh_array[ $p_status ] ) ) {
			...
			return config_get( 'update_candidate_status_threshold' );
		}
	}
?>
