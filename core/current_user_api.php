...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------
	# Return the number of open assigned candidates to the current user in
	#  the current project
	function current_user_get_assigned_open_candidate_count() {
		return user_get_assigned_open_candidate_count( auth_get_current_user_id(),
													helper_get_current_project() );
	}
	# --------------------
	# Return the number of open reported candidates by the current user in
	#  the current project
	function current_user_get_reported_open_candidate_count() {
		return user_get_reported_open_candidate_count( auth_get_current_user_id(),
													helper_get_current_project() );
	}
	# --------------------
	...
	# --------------------
	# return the candidate filter parameters for the current user
	function current_user_get_candidate_filter( $p_project_id = null ) {
		...
		if ( !is_blank( $f_filter_string ) ) {
			...
			$t_filter = user_get_candidate_filter( $t_user_id, $p_project_id );
		}
	...
	function current_user_get_interviewer_name( $p_name ) {
		$c_name		= db_prepare_string( $p_name );
		$t_interviewer_table = config_get( 'cosmos_project_version_table' );

		$query = "SELECT version 
				  FROM $t_interviewer_table
				  WHERE version like '%$c_name%'";
