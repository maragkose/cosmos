...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------
	# Checks the provided candidate and determines whether it should be included in the changelog
	# or not.
	# returns true: to include, false: to exclude.
	function custom_function_default_changelog_include_issue( $p_issue_id ) {
		$t_issue = candidate_get( $p_issue_id );

		return ( ( $t_issue->duplicate_id == 0 ) && ( $t_issue->resolution == FIXED ) &&
			( $t_issue->status >= config_get( 'candidate_resolved_status_threshold' ) ) );
	}
	...
	function custom_function_default_changelog_print_issue( $p_issue_id, $p_issue_level = 0 ) {
		$t_candidate = candidate_get( $p_issue_id );

		$t_category = is_blank( $t_candidate->category ) ? '' : '<b>[' . $t_candidate->category . ']</b> ';
		echo str_pad( '', $p_issue_level * 6, '&nbsp;' ), '- ', string_get_candidate_view_link( $p_issue_id ), ': ', $t_category, string_display_line_links( $t_candidate->summary );

		if ( $t_candidate->handler_id != 0 ) {
			echo ' (', prepare_user_name( $t_candidate->handler_id ), ')';
		}

		echo ' - ', get_enum_element( 'status', $t_candidate->status ), '.<br />';
	}

	# --------------------
	# Checks the provided candidate and determines whether it should be included in the roadmap or not.
	# returns true: to include, false: to exclude.
	...
	function custom_function_default_roadmap_print_issue( $p_issue_id, $p_issue_level = 0 ) {
		...
		if ( candidate_is_resolved( $p_issue_id ) ) {
		...
		$t_category = is_blank( $t_candidate->category ) ? '' : '<b>[' . $t_candidate->category . ']</b> ';
		echo str_pad( '', $p_issue_level * 6, '&nbsp;' ), '- ', $t_strike_start, string_get_candidate_view_link( $p_issue_id ), ': ', $t_category, string_display_line_links( $t_candidate->summary );

		if ( $t_candidate->handler_id != 0 ) {
			echo ' (', prepare_user_name( $t_candidate->handler_id ), ')';
		}

		echo ' - ', get_enum_element( 'status', $t_candidate->status ), $t_strike_end, '.<br />';
	}

	# --------------------
	# format the candidate summary.
	function custom_function_default_format_issue_summary( $p_issue_id, $p_context=0 ) {
		switch ( $p_context ) {
			case SUMMARY_CAPTION:
				$t_string = candidate_format_id( $p_issue_id ) . ': ' . string_attribute( candidate_get_field( $p_issue_id, 'summary' ) );
				break;
			case SUMMARY_FIELD:
				$t_string = candidate_format_id( $p_issue_id ) . ': ' . string_display_line_links( candidate_get_field( $p_issue_id, 'summary' ) );
				break;
			case SUMMARY_EMAIL:
				$t_string = candidate_format_id( $p_issue_id ) . ': ' . string_attribute( candidate_get_field( $p_issue_id, 'summary' ) );
				break;
			case SUMMARY_FIELD_SIMPLE:
				$t_string = string_attribute( candidate_get_field( $p_issue_id, 'summary' ) );
				break;
			default:
				$t_string = string_attribute( candidate_get_field( $p_issue_id, 'summary' ) );
				break;
	...
	function custom_function_default_checkin( $p_issue_id, $p_comment, $p_file, $p_new_version, $p_fixed ) {
		if ( candidate_exists( $p_issue_id ) ) {
			...
			candidatenote_add( $p_issue_id, $p_comment, 0, VS_PRIVATE == config_get( 'source_control_notes_view_status' ) );

			$t_status = config_get( 'source_control_set_status_to' );
			if ( ( OFF != $t_status ) && $p_fixed ) {
				candidate_set_field( $p_issue_id, 'status', $t_status );
				candidate_set_field( $p_issue_id, 'resolution', config_get( 'source_control_set_resolution_to' ) );
			}
		}
	}
	# --------------------
	# trigger helper function  
	function custom_function_default_trigger( $p_issue_id, $p_new_issue_data, $p_old_data, $p_handler_id ) {
		echo $p_new_issue_data->status;
		echo $p_old_data->status;
		if ($p_new_issue_data->status == CONFIRMED && $p_old_data->status == CONFIRMED) {
			$p_new_issue_data->fixed_in_version =  $p_handler_id;
		} 
	}
	...
	function custom_function_default_issue_update_validate( $p_issue_id, $p_new_issue_data, $p_candidatenote_text ) {
	}
	...
	# can COSMOS update the password
	...
	# The column names to use are those of the field names in the candidate table.
...
#}
	...
	# $p_column: custom_xxx for custom field xxx, or otherwise field name as in candidate table.
	# $p_columns_target: see COLUMNS_TARGET_* in constant_inc.php
	function custom_function_default_print_column_title( $p_column, $p_columns_target = COLUMNS_TARGET_VIEW_PAGE ) {
		...
		if ( strpos( $p_column, 'custom_' ) === 0 ) {
			...
			if ( $t_field_id === false ) {
				...
				if ( COLUMNS_TARGET_CSV_PAGE != $p_columns_target ) {
					print_view_candidate_sort_link( $t_custom_field, $p_column, $t_sort, $t_dir, $p_columns_target );
			...
			if ( function_exists( $t_function ) ) {
				...
				echo '<td>';
				print_view_candidate_sort_link( lang_get_defaulted( $p_column ), $p_column, $t_sort, $t_dir, $p_columns_target );
				print_sort_icon( $t_dir, $t_sort, $p_column );
				echo '</td>';
			}
		}
	...
	# $p_row: the row from the candidate table that belongs to the issue that we should print the values for.
	# $p_columns_target: see COLUMNS_TARGET_* in constant_inc.php
	function custom_function_default_print_column_value( $p_column, $p_issue_row, $p_columns_target = COLUMNS_TARGET_VIEW_PAGE ) {
		if ( COLUMNS_TARGET_CSV_PAGE == $p_columns_target ) {
			...
			if (strpos($p_column, config_get('column_containing_word'))){
				$t_column_start = '<td class="' . config_get('column_containing_word_css') . '">';
			} else {
				if ($p_column == 'sticky' && $p_issue_row[$p_column]==1){
					$t_column_start = '<td class="sticky">';
				} else {
		...
		if ( strpos( $p_column, 'custom_' ) === 0 ) {
			...
			if ( function_exists( $t_function ) ) {
				...
				if ( isset( $p_issue_row[$p_column] ) ) {
					if ($p_column == 'sticky'){
						echo $t_column_start . ($p_issue_row[$p_column] == 1 ? 'YES' : 'NO') . $t_column_end;
					} else {
	...
	# --------------------
	# This function prints the custom buttons on the current view page based on specified candidate id
	...
	function custom_function_default_print_candidate_view_page_custom_buttons( $p_candidate_id ) {
	}
# returns an array of the column names to be displayed.
# The column names to use are those of the field names in the candidate table.
# In addition, you can use the following:
