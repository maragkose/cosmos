...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------
	# Gets the filename without the candidate id prefix.
	function file_get_display_name( $p_filename ) {
		...
		# or a candidate attachment filename (0000000-filename)
		...
		if ( isset( $t_name[1] ) ) {
			...
			$t_candidate_match = '/^\d{7}-/';
			$t_name = preg_split($t_candidate_match, $p_filename);
			if ( isset( $t_name[1] ) ) {
				return $t_name[1];
			} else {
				return $p_filename;
	...
	# --------------------
	# Check the number of attachments a candidate has (if any)
	function file_candidate_attachment_count( $p_candidate_id ) {
		...
		if ( isset( $g_cache_file_count[ $p_candidate_id ] ))
			return $g_cache_file_count[ $p_candidate_id ];

		# If there is no cache hit, check if there is anything in
		#   the cache. If the cache isn't empty and we didn't have
		#   a hit, then there are not attachments for this candidate.
		if ( count( $g_cache_file_count ) > 0 )
			return 0;

		# Otherwise build the cache and return the attachment count
		#   for the given candidate (if any).
		$query = "SELECT candidate_id, COUNT(candidate_id) AS attachments
		...
		while( $row = db_fetch_array( $result )) {
			$g_cache_file_count[ $row['candidate_id'] ] = $row['attachments'];
			if ( $p_candidate_id == $row['candidate_id'] )
	...
	# --------------------
	# Check if a specific candidate has attachments
	function file_candidate_has_attachments( $p_candidate_id ) {
		if ( file_candidate_attachment_count( $p_candidate_id ) > 0 ) {
			return true;
		} else {
			return false;
		}
	...
	# --------------------
	# Check if the current user can view attachments for the specified candidate.
	function file_can_view_candidate_attachments( $p_candidate_id ) {
		$t_reported_by_me	= candidate_is_user_reporter( $p_candidate_id, auth_get_current_user_id() );
		$t_can_view     	= access_has_candidate_level( config_get( 'view_attachments_threshold' ), $p_candidate_id );
# @@@ Fix this to be readable
	...
	# --------------------
	# Check if the current user can download attachments for the specified candidate.
	function file_can_download_candidate_attachments( $p_candidate_id ) {
		$t_reported_by_me	= candidate_is_user_reporter( $p_candidate_id, auth_get_current_user_id() );
		$t_can_download		= access_has_candidate_level( config_get( 'download_attachments_threshold' ), $p_candidate_id );
# @@@ Fix this to be readable
	...
	# --------------------
	# Check if the current user can delete attachments from the specified candidate.
	function file_can_delete_candidate_attachments( $p_candidate_id ) {
		if ( candidate_is_readonly( $p_candidate_id ) ) {
			return false;
		}

		$t_reported_by_me	= candidate_is_user_reporter( $p_candidate_id, auth_get_current_user_id() );
		$t_can_download		= access_has_candidate_level( config_get( 'delete_attachments_threshold' ), $p_candidate_id );
# @@@ Fix this to be readable
	...
	# --------------------
	# List the attachments belonging to the specified candidate.  This is used from within
	# candidate_view_page.php and candidate_view_advanced_page.php
	function file_list_attachments( $p_candidate_id ) {
		$t_attachment_rows = candidate_get_attachments( $p_candidate_id );
		...
		$t_can_download = file_can_download_candidate_attachments( $p_candidate_id );
		$t_can_delete   = file_can_delete_candidate_attachments( $p_candidate_id );
		...
		for ( $i = 0 ; $i < $num_files ; $i++ ) {
			...
			if ( $t_can_download ) {
				...
				$t_href_clicket = " [<a href=\"file_download.php?file_id=$v_id&amp;type=candidate\" target=\"_blank\">^</a>]";
			} else {
				$t_href_start	= '';
				$t_href_end		= '';
			...
			if ( !$t_exists ) {
				...
				if ( $t_can_delete ) {
					PRINT " [<a class=\"small\" href=\"candidate_file_delete.php?file_id=$v_id\">" . lang_get('delete_link') . '</a>]';
				}
...
document.getElementById( span ).style.display = displayType;
 ...
 -->
				...
				if ( $t_can_download &&
					...
					PRINT "\n<br />$t_href_start<img alt=\"$t_title\" $t_preview_style src=\"file_download.php?file_id=$v_id&amp;type=candidate\" />$t_href_end";
	...
	# --------------------
	# delete all files that are associated with the given candidate
	function file_delete_attachments( $p_candidate_id ) {
		$c_candidate_id = db_prepare_int( $p_candidate_id );
		...
		$query = "DELETE FROM $t_candidate_file_table
				  WHERE candidate_id='$c_candidate_id'";
		$result = db_query( $query );
	...
	function file_delete_project_files( $p_project_id ) {
		$t_project_file_table	= config_get( 'cosmos_project_file_table' );
		$t_method				= config_get( 'file_upload_method' );
	...
	function file_get_field( $p_file_id, $p_field_name, $p_table = 'candidate' ) {
		$c_file_id			= db_prepare_int( $p_file_id );
		$c_field_name		= db_prepare_string( $p_field_name );
		$t_candidate_file_table	= config_get( 'cosmos_' . $p_table . '_file_table' );
	...
	# --------------------
	function file_delete( $p_file_id, $p_table = 'candidate' ) {
		...
		if( 'candidate' == $p_table ) {
			# log file deletion
			$t_candidate_id			= file_get_field( $p_file_id, 'candidate_id', 'candidate' );
			history_log_event_special( $t_candidate_id, FILE_DELETED, file_get_display_name ( $t_filename ) );
		}

		$t_file_table	= config_get( 'cosmos_' . $p_table . '_file_table' );
	...
	function diskfile_is_name_unique( $p_name , $p_filepath ) {
		$t_file_table = config_get( 'cosmos_candidate_file_table' );
	...
	function file_is_name_unique( $p_name, $p_candidate_id ) {
		$t_file_table = config_get( 'cosmos_candidate_file_table' );

		$c_name = db_prepare_string( $p_name );
		$c_candidate = db_prepare_string( $p_candidate_id );

		$query = "SELECT COUNT(*)
				  FROM $t_file_table
				  WHERE filename='$c_name' and candidate_id=$c_candidate";
		$result = db_query( $query );
		$t_count = db_result( $result );
	...
	# --------------------
	function file_add( $p_candidate_id, $p_tmp_file, $p_file_name, $p_file_type='', $p_table = 'candidate', $p_file_error = 0, $p_title = '', $p_desc = '' ) {
		...
		if ( !file_is_name_unique( $p_file_name, $p_candidate_id ) ) {
			trigger_error( ERROR_DUPLICATE_FILE, ERROR );
		}

		if ( 'candidate' == $p_table ) {
			$t_project_id	= candidate_get_field( $p_candidate_id, 'project_id' );
			$t_candidate_id		= candidate_format_id( $p_candidate_id );
		} else {
		...
		$t_file_hash = ( 'candidate' == $p_table ) ? $t_candidate_id : config_get( 'document_files_prefix' ) . '-' . $t_project_id;
		...
		$t_file_table	= config_get( 'cosmos_' . $p_table . '_file_table' );
		$c_id = ( 'candidate' == $p_table ) ? $c_candidate_id : $c_project_id;
		...
		if ( 'candidate' == $p_table ) {
			...
			$result = candidate_update_date( $p_candidate_id );

			# log new candidate
			history_log_event_special( $p_candidate_id, FILE_ADDED, $p_file_name );
		}
	...
	# --------------------
	# Check if the user can upload files for this candidate
	#  return true if they can, false otherwise
	#  the user defaults to the current user
	#
	#  if the candidate null (the default) we answer whether the user can
	#   upload a file to a new candidate in the current project
	function file_allow_candidate_upload( $p_candidate_id = null, $p_user_id = null ) {
		if ( null === $p_user_id ) {
			$p_user_id = auth_get_current_user_id();
		}
		...
		if ( null === $p_candidate_id ) {		# new candidate
			$t_project_id = helper_get_current_project();

			# the user must be the reporter if they're reporting a new candidate
			$t_reporter = true;
		} else {						# existing candidate
			$t_project_id = candidate_get_field( $p_candidate_id, 'project_id' );

			# check if the user is the reporter of the candidate
			$t_reporter = candidate_is_user_reporter( $p_candidate_id, $p_user_id );
		}
        ...
        return access_has_project_level( config_get( 'upload_candidate_file_threshold' ), $t_project_id, $p_user_id );
	}
