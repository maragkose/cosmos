...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	define( 'FILTER_PROPERTY_RELATIONSHIP_BUG', 'relationship_candidate' );
	...
	define( 'FILTER_SEARCH_RELATIONSHIP_BUG', 'relationship_candidate' );
	...
	# $p_per_page
	#   - the number of candidates to see per page (set to actual on return)
	#     -1   indicates you want to see all candidates
	...
	# $p_candidate_count
	#   - you don't need to give a value here, the number of candidates will be
	#     stored here on return
	# $p_custom_filter
	#   - Filter to use.
	# $p_project_id
	...
	function filter_get_candidate_rows( &$p_page_number, &$p_per_page, &$p_page_count, &$p_candidate_count, $p_custom_filter = null, $p_project_id = null, $p_user_id = null, $p_show_sticky = null ) {
		...
		$t_candidate_relationship_table	= config_get( 'cosmos_candidate_relationship_table' );
		$t_report_candidate_threshold		= config_get( 'report_candidate_threshold' );
		...
		if ( $p_custom_filter === null ) {
			# Prefer current_user_get_candidate_filter() over user_get_filter() when applicable since it supports
			# cookies set by previous version of the code.
			if ( $t_user_id == $t_current_user_id ) {
				$t_filter = current_user_get_candidate_filter();
			} else {
				$t_filter = user_get_candidate_filter( $t_user_id, $t_project_id );
			}
		} else {
		...
		$t_where_clauses = array( "$t_project_table.enabled = 1", "$t_project_table.id = $t_candidate_table.project_id" );
		$t_select_clauses = array( "$t_candidate_table.*" );
		...
		if ( $t_projects_query_required ) {
			...
			$t_access_required_to_view_private_candidates = config_get( 'private_candidate_threshold' );
			foreach ( $t_project_ids as $t_pid ) {
				if ( access_has_project_level( $t_access_required_to_view_private_candidates, $t_pid, $t_user_id ) ) {
			...
			if ( $t_count_private_and_public_project_ids == 1 ) {
				$t_private_and_public_query = "( $t_candidate_table.project_id = " . $t_private_and_public_project_ids[0] . " )";
			} else if ( $t_count_private_and_public_project_ids > 1 ) {
				$t_private_and_public_query = "( $t_candidate_table.project_id in (". implode( ', ', $t_private_and_public_project_ids ) . ") )";
			} else {
				$t_private_and_public_query = null;
			}

			$t_count_public_only_project_ids = count( $t_public_only_project_ids );
			$t_public_view_state_check = "( ( $t_candidate_table.view_state = " . VS_PUBLIC . " ) OR ( $t_candidate_table.reporter_id = $t_user_id ) )";
			if ( $t_count_public_only_project_ids == 1 ) {
				$t_public_only_query = "( ( $t_candidate_table.project_id = " . $t_public_only_project_ids[0] . " ) AND $t_public_view_state_check )";
			} else if ( $t_count_public_only_project_ids > 1 ) {
				$t_public_only_query = "( ( $t_candidate_table.project_id in (". implode( ', ', $t_public_only_project_ids ) . ") ) AND $t_public_view_state_check )";
			} else {
		...
		if ( ( $t_filter['view_state'] !== META_FILTER_ANY ) && ( !is_blank( $t_filter['view_state'] ) ) ) {
			$t_view_state_query = "($t_candidate_table.view_state='$t_view_state')";
		...
		if ( !$t_any_found ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				$t_reporter_query = "( $t_candidate_table.reporter_id in (". implode( ', ', $t_clauses ) .") )";
			} else {
				$t_reporter_query = "( $t_candidate_table.reporter_id=$t_clauses[0] )";
			}
		...
		if ( ( ON === $t_limit_reporters ) && ( ! access_has_project_level( REPORTER + 1, $t_project_id, $t_user_id ) ) ) {
			...
			array_push( $t_where_clauses, "($t_candidate_table.reporter_id='$c_reporter_id')" );
		}
		...
		if ( !$t_any_found ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				$t_handler_query = "( $t_candidate_table.handler_id in (". implode( ', ', $t_clauses ) .") )";
			} else {
				$t_handler_query = "( $t_candidate_table.handler_id=$t_clauses[0] )";
			}
		...
		if ( !_filter_is_any( $t_filter['show_category'] ) ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.category in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.category=$t_clauses[0] )" );
			}
		}
		...
		if ( !$t_any_found ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.severity in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.severity=$t_clauses[0] )" );
			}
		}
		...
		if ( count( $t_desired_statuses ) > 0 ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.status in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.status=$t_clauses[0] )" );
			}
		}
		...
		if ( !$t_any_found ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.resolution in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.resolution=$t_clauses[0] )" );
			}
		}
		...
		if ( !$t_any_found ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.priority in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.priority=$t_clauses[0] )" );
			}
		}
		...
		if ( !$t_any_found ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.build in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.build=$t_clauses[0] )" );
			}
		}
		...
		if ( !_filter_is_any( $t_filter['show_version'] ) ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.version in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.version=$t_clauses[0] )" );
			}
		}
		...
		if ( !_filter_is_any( $t_filter['show_profile'] ) ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.profile_id in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.profile_id=$t_clauses[0] )" );
			}
		}
		...
		if ( !_filter_is_any( $t_filter['platform'] ) ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.platform in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.platform = $t_clauses[0] )" );
			}
		}
		...
		if ( !_filter_is_any( $t_filter['os'] ) ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.os in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.os = $t_clauses[0] )" );
			}
		}
		...
		if ( !_filter_is_any( $t_filter['os_build'] ) ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.os_build in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.os_build = $t_clauses[0] )" );
			}
		}
		...
		if ( ( 'on' == $t_filter['do_filter_by_date'] ) &&
			...
			array_push( $t_where_clauses, "($t_candidate_table.date_submitted BETWEEN '$t_start_string' AND '$t_end_string' )" );
		}
		...
		if ( !_filter_is_any( $t_filter['fixed_in_version'] ) ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.fixed_in_version in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.fixed_in_version=$t_clauses[0] )" );
			}
		}
		...
		if ( !_filter_is_any( $t_filter['target_version'] ) ) {
			...
			if ( 1 < count( $t_clauses ) ) {
				array_push( $t_where_clauses, "( $t_candidate_table.target_version in (". implode( ', ', $t_clauses ) .") )" );
			} else {
				array_push( $t_where_clauses, "( $t_candidate_table.target_version=$t_clauses[0] )" );
			}
		}

		# users monitoring a candidate
		$t_any_found = false;
		...
		if ( !$t_any_found ) {
			...
			array_push( $t_from_clauses, $t_candidate_monitor_table );
			array_push( $t_join_clauses, "LEFT JOIN $t_candidate_monitor_table $t_table_name ON $t_table_name.candidate_id = $t_candidate_table.id" );
		...
		$c_rel_candidate = $t_filter['relationship_candidate'];
		if ( -1 == $c_rel_type || 0 == $c_rel_candidate) {
			$t_any_found = true;
		}
		if ( !$t_any_found ) {
			...
			array_push( $t_from_clauses, $t_candidate_relationship_table );
			array_push( $t_join_clauses, "LEFT JOIN $t_candidate_relationship_table $t_table_name ON $t_table_name.destination_candidate_id = $t_candidate_table.id" );
			array_push( $t_join_clauses, "LEFT JOIN $t_candidate_relationship_table ${t_table_name}2 ON ${t_table_name}2.source_candidate_id = $t_candidate_table.id" );
			// get reverse relationships
 			array_push( $t_clauses, "($t_table_name.relationship_type='$t_comp_type' AND $t_table_name.source_candidate_id='$c_rel_candidate')" );
			array_push( $t_clauses, "($t_table_name"."2.relationship_type='$c_rel_type' AND $t_table_name"."2.destination_candidate_id='$c_rel_candidate')" );
		...
		if ( !is_blank( $c_tag_string ) ) {
			...
			if ( count( $t_tags ) ) {
				...
				$t_candidate_tag_table = config_get( 'cosmos_candidate_tag_table' );
				
				if ( count( $t_tags_all ) ) {
					$t_clauses = array();
					foreach ( $t_tags_all as $t_tag_row ) {
						array_push( $t_clauses, "$t_candidate_table.id IN ( SELECT candidate_id FROM $t_candidate_tag_table WHERE $t_candidate_tag_table.tag_id = $t_tag_row[id] )" );
					}
				...
				if ( count( $t_tags_any ) ) {
					...
					foreach ( $t_tags_any as $t_tag_row ) {
						array_push( $t_clauses, "$t_candidate_tag_table.tag_id = $t_tag_row[id]" );
					}
					array_push( $t_where_clauses, "$t_candidate_table.id IN ( SELECT candidate_id FROM $t_candidate_tag_table WHERE ( ". implode( ' OR ', $t_clauses ) .') )' );
				}
				...
				if ( count( $t_tags_none ) ) {
					...
					array_push( $t_where_clauses, "$t_candidate_table.id NOT IN ( SELECT candidate_id FROM $t_candidate_tag_table WHERE ( ". implode( ' OR ', $t_clauses ) .') )' );
				} 
		...
		if( ON == config_get( 'filter_by_custom_fields' ) ) {
			...
			foreach( $t_custom_fields as $t_cfid ) {
				...
				if ( !$t_any_found ) {
					...
					$t_cf_join_clause = "LEFT JOIN $t_custom_field_string_table $t_table_name ON $t_table_name.candidate_id = $t_candidate_table.id AND $t_table_name.field_id = $t_cfid ";
					...
					if ($t_def['type'] == CUSTOM_FIELD_TYPE_DATE) {
						...
						case CUSTOM_FIELD_DATE_NONE:
							...
							$t_custom_where_clause = '(( ' . $t_table_name . '.candidate_id is null) OR ( ' . $t_table_name . '.value = 0)' ;
							break ;
						...
						foreach( $t_filter['custom_fields'][$t_cfid] as $t_filter_member ) {
							...
							if ( META_FILTER_NONE == $t_filter_member ) { 
								...
								array_push( $t_filter_array , "$t_candidate_table.id NOT IN (SELECT candidate_id FROM $t_custom_field_string_table WHERE field_id=$t_cfid)" );
							}
		...
		if ( !is_blank( $t_filter['search'] ) ) {
			...
			$t_textsearch_where_clause = '(' . db_helper_like( 'summary', "%$c_search%" ) .
							 ' OR ' . db_helper_like( "$t_candidate_text_table.description", "%$c_search%" ) . 
							 ' OR ' . db_helper_like( "$t_candidate_text_table.steps_to_reproduce", "%$c_search%" ) .
							 ' OR ' . db_helper_like( "$t_candidate_text_table.additional_information", "%$c_search%" ) .
							 " OR ( $t_candidate_table.id = '$c_search_int' ) )";

			$t_textsearch_wherejoin_clause = '(' . db_helper_like( 'summary', "%$c_search%" ) .
							 ' OR ' . db_helper_like( "$t_candidate_text_table.description", "%$c_search%" ) .
							 ' OR ' . db_helper_like( "$t_candidate_text_table.steps_to_reproduce", "%$c_search%" ) .
							 ' OR ' . db_helper_like( "$t_candidate_text_table.additional_information", "%$c_search%" ) .
							 ' OR ' . db_helper_like( "$t_candidate_table.id", "%$c_search%" ) .
							 ' OR ' . db_helper_like( "$t_candidatenote_text_table.note", "%$c_search%" ) . ' )';

			array_push( $t_where_clauses, "($t_candidate_text_table.id = $t_candidate_table.candidate_text_id)" );

			$t_from_clauses = array( $t_candidate_text_table, $t_project_table, $t_candidate_table );
		} else {
			$t_from_clauses = array( $t_project_table, $t_candidate_table );
		}
		...
		# have candidatenotes that match the text search if necessary.
		$t_id_array = array();
		for ( $i = 0; $i < 2; $i++ ) {
			$t_id_where = $t_where;
			$t_id_join = $t_join;
			if ( $i == 0 ) {
				...
				$t_id_join = $t_id_join . " INNER JOIN $t_candidatenote_table ON $t_candidatenote_table.candidate_id = $t_candidate_table.id";
				$t_id_join = $t_id_join . " INNER JOIN $t_candidatenote_text_table ON $t_candidatenote_text_table.id = $t_candidatenote_table.candidatenote_text_id";
			}
			$query  = "SELECT DISTINCT $t_candidate_table.id AS id
						$t_from
						$t_id_join
						$t_id_where";
		...
		# Get the total number of candidates that meet the criteria.
		$candidate_count = count( $t_id_array );

		$rows = array();

		if ( $candidate_count > 0 ) {
			$t_where = "WHERE $t_candidate_table.id in (" . implode( ", ", $t_id_array ) . ")";
		} else {
			return $rows;
		}

		$t_from = 'FROM ' . $t_candidate_table;
		...
		if ( null === $p_per_page ) {
			...
			$p_per_page = $candidate_count;
		}

		# Guard against silly values of $f_per_page.
		if ( 0 == $p_per_page ) {
			$p_per_page = $candidate_count;	// 0 - means show all
		}

		$p_per_page = (int)abs( $p_per_page );

		# Use $candidate_count and $p_per_page to determine how many pages
		# to split this list up into.
		# For the sake of consistency have at least one page, even if it
		# is empty.
		$t_page_count = ceil($candidate_count / $p_per_page);
		if ( $t_page_count < 1 ) {
			$t_page_count = 1;
		}
		...
		for ( $i=0; $i < count( $t_sort_fields ); $i++ ) {
			...
			if ( ! in_array( $t_sort_fields[$i], array_slice( $t_sort_fields, $i + 1) ) ) {
        		...
        		if ( strpos( $c_sort, 'custom_' ) === 0 ) {
    	    		...
    	    		$t_join .= " LEFT JOIN $t_custom_field_string_table ON ( ( $t_custom_field_string_table.candidate_id = $t_candidate_table.id ) AND ( $t_custom_field_string_table.field_id = $t_custom_field_id ) )";
		...
		$t_where = "WHERE $t_candidatenote_table.candidate_id in (" . implode( ", ", $t_id_array_lastmod ) . ")";
		
		$query3 = "SELECT DISTINCT candidate_id,MAX(last_modified) as last_modified, COUNT(last_modified) as count FROM $t_candidatenote_table $t_where GROUP BY candidate_id";
		...
		for ( $i=0 ; $i < $row_count ; $i++ ) {
			...
			$t_stats[ $row['candidate_id'] ] = $row;
		}

		foreach($rows as $row) {
			if( !isset( $t_stats[ $row['id'] ] ) ) {
				candidate_cache_database_result( $row, false );
			} else {
				candidate_cache_database_result( $row, $t_stats[ $row['id'] ] );
			}
		}
	...
	# --------------------
	# Will print the filter selection area for both the candidate list view screen, as well
	# as the candidate list print screen. This function was an attempt to make it easier to
...
?>
								...
								if ( count( $t_filter['user_monitor'] ) == 0 ) {
									...
									foreach( $t_filter['user_monitor'] as $t_current ) {
										...
										if ( ( $t_current === 0 ) || ( is_blank( $t_current ) ) || ( META_FILTER_ANY == $t_current ) ) {
											...
											if ( access_has_project_level( config_get( 'monitor_candidate_threshold' ) ) ) {
												$t_this_name = '[' . lang_get( 'myself' ) . ']';
											} else {
												$t_any_found = true;
											}
								...
								if ( count( $t_filter['handler_id'] ) == 0 ) {
									...
									foreach( $t_filter['handler_id'] as $t_current ) {
										...
										if ( META_FILTER_NONE == $t_current ) {
											...
											if ( access_has_project_level( config_get( 'handle_candidate_threshold' ) ) ) {
												$t_this_name = '[' . lang_get( 'myself' ) . ']';
											} else {
												$t_any_found = true;
											}
		...
		<tr <?php PRINT "class=\"" . $t_trclass . "\""; ?>>
			...
			<td class="small-caption" valign="top" colspan="2">
				<a href="<?php PRINT $t_filters_url . 'relationship_type'; ?>" id="relationship_type_filter"><?php PRINT lang_get( 'candidate_relationships' ) ?>:</a>
			</td>
			<?php if ( $t_filter_cols > 8 ) {
				echo '<td class="small-caption" valign="top" colspan="' . ( $t_filter_cols - 8 ) . '">&nbsp;</td>';
			} ?>
		...
		<tr class="row-1">
			...
			<td class="small-caption" valign="top" colspan="2" id="relationship_type_filter_target">
							...
							<input type="hidden" name="relationship_candidate" value="<?php echo $t_filter['relationship_candidate'];?>" />
							<?php
								$c_rel_type = $t_filter['relationship_type'];
								$c_rel_candidate = $t_filter['relationship_candidate'];
								if ( -1 == $c_rel_type || 0 == $c_rel_candidate ) {
								    ...
								    PRINT relationship_get_description_for_history ($c_rel_type) . ' ' . $c_rel_candidate;
								}
				...
				<?php
					...
					for ( $i=0; $i<2; $i++ ) {
						if ( isset( $t_sort_fields[$i] ) ) {
							...
							echo $t_field_name . " " . lang_get( 'candidatenote_order_' . strtolower( $t_dir_fields[$i] ) );
			...
			<td class="center" colspan="<?php echo ( $t_filter_cols - 6 ) ?>"> <!-- use this label for padding -->
				<?php
					...
					if ( ( SIMPLE_ONLY != config_get( 'view_filters' ) ) && ( ADVANCED_ONLY != config_get( 'view_filters' ) ) ) {
						if ( 'advanced' == $t_view_type ) {
							print_button_link( $f_switch_view_link . 'simple', lang_get( 'simple_filters' ), 'button-small' );
						} else {
							print_button_link( $f_switch_view_link . 'advanced', lang_get( 'advanced_filters' ), 'button-small');
						}

						print_button_link( 
							'permalink_page.php?url=' . urlencode( filter_get_url( $t_filter ) ), 
							lang_get( 'create_filter_link' ), 'button-small', 
							/* new window = */ true );
...
<?php
	...
	function filter_db_set_for_current_user( $p_project_id, $p_is_public,
		...
		$t_filters_table = config_get( 'cosmos_filters_table' );
	...
	function filter_ensure_valid_filter( $p_filter_arr ) {
		...
		if ( !isset( $p_filter_arr['relationship_candidate'] ) ) {
			$p_filter_arr['relationship_candidate'] = gpc_get_int( 'relationship_candidate', 0 );
		}
		...
		for ( $i=0; $i < $t_n_fields; $i++ ) {
			if ( isset( $t_fields[$i] ) && in_array( $t_fields[$i], array( 'selection', 'edit', 'candidatenotes_count', 'attachment' ) ) ) {
				unset( $t_fields[$i] );
			}
		}
	...
	function print_filter_sticky_issues(){
		global $t_filter;
		?><!-- Show or hide sticky candidates -->
	...
	function print_filter_relationship_type(){
		...
		<input type="text" name="relationship_candidate" size="5" maxlength="10" value="<?php echo $t_filter['relationship_candidate']?>" />
		<?php
	...
	function print_filter_show_sort() {
		...
		$t_shown_fields[""] = "";
		for ( $i=0; $i < $t_n_fields; $i++ ) {
			if ( !in_array( $t_fields[$i], array( 'selection', 'edit', 'candidatenotes_count', 'attachment' ) ) ) {
		...
		$t_shown_dirs[""] = "";
		$t_shown_dirs["ASC"] = lang_get( 'candidatenote_order_asc' );
		$t_shown_dirs["DESC"] = lang_get( 'candidatenote_order_desc' );
		...
		if ( count( $t_fields ) > 0 ) {
			...
			echo lang_get_defaulted( 'last_updated' ) . lang_get( 'candidatenote_order_desc' );
