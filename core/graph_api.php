...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# graphing routines
	# --------------------
	function graph_bar( $p_metrics, $p_title='', $p_graph_width = 350, $p_graph_height = 400, $horizontal=false){

		#$t_graph_font = graph_get_font();
		$t_graph_font = FF_VERA;

		error_check( is_array( $p_metrics ) ? array_sum( $p_metrics ) : 0, $p_title );

		$graph = new Graph( $p_graph_width, $p_graph_height );
		$graph->img->SetMargin(90,90,90,200);
		#$graph->img->SetMargin(40,40,40,200);
		...
		if ($horizontal==true){
			...
			$graph->Set90AndMargin(250,120,70,30);
			$p1 = new BarPlot( array_values( $p_metrics ) );
			$p1->SetFillGradient("black","darkorange",GRAD_MIDVER);
		} else {
			$p1 = new BarPlot( array_values( $p_metrics ) );
			$p1->SetFillGradient("black","purple",GRAD_MIDVER);
		}
		$p1->SetWidth(0.8);
		$p1->value->show();
		...
		if ( helper_show_queries() ) {
			...
			$graph->subtitle->SetFont( $t_graph_font, FF_VERA, 8 );
		}
	...
	function graph_group_pct( $p_title='', $p_graph_width = 350, $p_graph_height = 400 ){
		...
		global $open_candidate_count, $closed_candidate_count, $resolved_candidate_count;

		error_check( $open_candidate_count + $closed_candidate_count + $resolved_candidate_count, $p_title );
		...
		$p1 = new BarPlot($open_candidate_count);
		$p1->SetFillColor('yellow');
		$p1->SetWidth(0.8);
		$p1->SetLegend( lang_get( 'legend_opened' ) );

		$p2 = new BarPlot($closed_candidate_count);
		$p2->SetFillColor('blue');
		$p2->SetWidth(0.8);
		$p2->SetLegend( lang_get( 'legend_closed' ) );

		$p3 = new BarPlot($resolved_candidate_count);
		$p3->SetFillColor('red');
		$p3->SetWidth(0.8);
		$p3->SetLegend( lang_get( 'legend_resolved' ) );
	...
	function graph_pie( $p_metrics, $p_title='',
		...
		#$p1->SetLabels( array_keys( $p_metrics ) );
	...
	# --------------------
	# --------------------
	# summarize metrics by a single field in the candidate table
	function create_candidate_enum_summary( $p_enum_string, $p_enum ) {
	...
	function enum_candidate_group( $p_enum_string, $p_enum ) {
		$t_candidate_table = config_get( 'cosmos_candidate_table' );

		$t_project_id = helper_get_current_project();
		$t_candidate_table = config_get( 'cosmos_candidate_table' );
		$t_user_id = auth_get_current_user_id();
		$t_res_val = config_get( 'candidate_resolved_status_threshold' );
		$t_clo_val = CLOSED;
		$specific_where = " AND " . helper_project_specific_where( $t_project_id, $t_user_id );
		...
		for ( $i=0; $i < $enum_count; $i++) {
			...
			# Calculates the number of candidates opened and puts the results in a table
			$query = "SELECT COUNT(*)
					FROM $t_candidate_table
			...
			# Calculates the number of candidates closed and puts the results in a table
			$query = "SELECT COUNT(*)
					FROM $t_candidate_table
					WHERE $p_enum='$t_s[0]' AND
			...
			# Calculates the number of candidates resolved and puts the results in a table
			$query = "SELECT COUNT(*)
					FROM $t_candidate_table
					WHERE $p_enum='$t_s[0]' AND
	...
	# Function which gives the absolute values according to the status (opened/closed/resolved)
	function pipeline_stats($type='pie') {
		$t_candidate_table = config_get( 'cosmos_candidate_table' );

		$t_project_id = helper_get_current_project();
		$t_candidate_table = config_get( 'cosmos_candidate_table' );
		$t_custom_table = config_get( 'cosmos_custom_field_string_table' );
		$t_user_id = auth_get_current_user_id();
		$t_clo_val = CLOSED;
		$specific_where = helper_project_specific_where( $t_project_id, $t_user_id );

		$total_query = "SELECT COUNT(*) FROM $t_candidate_table WHERE $specific_where";
		$total_result = db_query( $total_query );
		$total =  db_result( $total_result, 0, 0);

		# Get the number of REJECTED from the assessment. Custom fied "Tool Suggestion" == REJECTED 
		$assessment_rejected_query = "SELECT COUNT(*) FROM $t_custom_table  where field_id = 53 and value = 'REJECTED'";
		$assessment_rejected_result = db_query( $assessment_rejected_query );
		$a_rejected =  db_result( $assessment_rejected_result, 0, 0);
		
		$assessment_passed_query = "SELECT COUNT(*) FROM $t_custom_table  where field_id = 53 and value = 'ELIGIBLE'";
		$assessment_passed_result = db_query( $assessment_passed_query );
		$a_passed =  db_result( $assessment_passed_result, 0, 0);

		$first_i_rejected_query = "SELECT COUNT(*) FROM $t_custom_table  INNER JOIN $t_candidate_table on $t_candidate_table.id = $t_custom_table.candidate_id AND $t_candidate_table.project_id  = 1 AND $t_custom_table.field_id = 22 AND $t_custom_table.value = 'Rejected' AND $specific_where";
		$first_i_rejected_result = db_query( $first_i_rejected_query );
		$first_rejected = db_result( $first_i_rejected_result, 0, 0);
		
		$second_i_ok_query = "SELECT COUNT(*) FROM $t_custom_table  INNER JOIN $t_candidate_table on $t_candidate_table.id = $t_custom_table.candidate_id AND $t_candidate_table.project_id  = 1 AND $t_custom_table.field_id = 22 AND $t_custom_table.value = 'Proceed to 2nd Interview' AND $specific_where";
		$second_i_ok_result = db_query( $second_i_ok_query );
		$first_passed = db_result( $second_i_ok_result, 0, 0);
		
		$second_i_rejected_query = "SELECT COUNT(*) FROM $t_custom_table  INNER JOIN $t_candidate_table on $t_candidate_table.id = $t_custom_table.candidate_id AND $t_candidate_table.project_id  = 1 AND $t_custom_table.field_id = 23 AND $t_custom_table.value = 'Rejected' AND $specific_where";
		$second_i_rejected_result = db_query( $second_i_rejected_query );
		$second_rejected = db_result( $second_i_rejected_result, 0, 0);
		
		$selected_query = "SELECT COUNT(*) FROM $t_candidate_table  where resolution = 20 AND $specific_where";
		$selected_result = db_query( $selected_query );
		$selected = db_result( $selected_result, 0, 0);
		
		$cv_query = "SELECT COUNT(*) FROM $t_candidate_table  where resolution = 90 AND $specific_where";
		$cv_result = db_query( $cv_query );
		$cv = db_result( $cv_result, 0, 0);
		
		$own_query = "SELECT COUNT(*) FROM $t_candidate_table  where resolution = 60 AND $specific_where";
		$own_result = db_query( $own_query );
		$own = db_result( $own_result, 0, 0);
		
		$onhold_query = "SELECT COUNT(*) FROM $t_candidate_table  where resolution = 40 AND $specific_where";
		$onhold_result = db_query( $onhold_query );
		$onhold = db_result( $onhold_result, 0, 0);
		
		$closed_query = "SELECT COUNT(*) FROM $t_candidate_table  where status = 90 AND $specific_where";
		$closed_result = db_query( $closed_query );
		$closed = db_result( $closed_result, 0, 0);

		#$processing = $total - $a_rejected - $first_rejected - $second_rejected - $selected - $onhold -$cv -$own;
		$processing = $total - $closed;
		if($type == 'pie'){
			$t_metrics['Rejected from CV Screen'] = ($cv/($total - $processing)) * 100;
			$t_metrics['Rejected from Assessment'] = ($a_rejected/($total - $processing)) * 100;
			$t_metrics['Rejected from First Interview'] = ($first_rejected/($total-$processing)) * 100;
			$t_metrics['Rejected from Second Interview'] = (($second_rejected/($total-$processing)) * 100); 
			$t_metrics['Selected/On Hold'] = (($selected+$onhold)/($total - $processing)) * 100; 
		} else {
			$t_metrics['TOTAL'] = $total;
			$t_metrics['Still Processing'] = $processing;
			$t_metrics['Rejected from CV Screen'] = $cv ;
			$t_metrics['Rejected from Assessment'] = $a_rejected;
			$t_metrics['Rejected from First Interview'] = $first_rejected;
			$t_metrics['Rejected from Second Interview'] = $first_passed - $second_rejected; 
			$t_metrics['Assessment Passed'] = $a_passed; 
			$t_metrics['First Interview Passed'] = $first_passed; 
			$t_metrics['Selected/On Hold'] = $selected + $onhold; 
			$t_metrics['Rejected from CV Screen (%)'] = ($cv/($total - $processing)) * 100;
			$t_metrics['Rejected from Assessment (%)'] = $a_rejected/($total - $processing -$cv) * 100;
			$t_metrics['Rejected from 1st Intv. vs Input (%)'] = $first_rejected/$a_passed * 100;
			$t_metrics['Rejected from 2nd Intv. vs Input (%)'] = $second_rejected/$first_passed * 100;
			$t_metrics['Overall Selection Rate (%)'] =( ($selected + $onhold)/($total - $processing)) * 100;
		}

		log_event (LOG_EMAIL, "total=$total, a_rejected = $a_rejected, first_rejected = $first_rejected, second_rejected = $second_rejected, first_passed =$first_passed, selected = $selected");
		return $t_metrics;
	}
	...
	function create_interviewer_summary() {

		$t_project_id = helper_get_current_project();
		$t_user_table = config_get( 'cosmos_project_version_table' );
		$t_candidate_table = config_get( 'cosmos_candidate_table' );
		$t_user_id = auth_get_current_user_id();
		$specific_where = " AND " . helper_project_specific_where( $t_project_id, $t_user_id );

		$query = "SELECT fixed_in_version 
				 FROM $t_candidate_table
				 WHERE id != '' $specific_where";
		$result = db_query( $query );
		$t_total_reported = db_num_rows( $result );

		$t_interviewer_arr = array();
		for ( $i = 0; $i < $t_total_reported; $i++ ) {
			$row = db_fetch_array( $result );

			if ( isset( $t_interviewer_arr[$row['fixed_in_version']] ) ) {
				$t_interviewer_arr[$row['fixed_in_version']]++;
			} else {
				$t_interviewer_arr[$row['fixed_in_version']] = 1;
			}
		}

		if ( count( $t_interviewer_arr ) == 0 ) {
			return array();
		}
		$t_imploded_interviewers = implode( '\',\'', array_keys( $t_interviewer_arr ) );
		$query = "SELECT id,version 
				FROM $t_user_table
				WHERE version IN ('$t_imploded_interviewers')
				ORDER BY version";
		$result = db_query( $query );
		$user_count = db_num_rows( $result );
		...
		for ($i=0;$i<$user_count;$i++) {
			...
			#$t_metrics[$v_id] = $t_interviewer_arr[$v_version];
			$t_metrics[$v_version] = $t_interviewer_arr[$v_version];
		} # end for
		return $t_metrics;
	}
	function create_second_interviewer_summary() {
		...
		for ( $i = 0; $i < $t_total_reported; $i++ ) {
			...
			if ( isset( $t_interviewer_arr[$row['target_version']] ) ) {
				$t_interviewer_arr[$row['target_version']]++;
			} else {
				$t_interviewer_arr[$row['target_version']] = 1;
			}
		}
	...
	function create_interviewer_summary_total() {
		...
		$query2 = "SELECT target_version 
				 FROM $t_candidate_table
				 WHERE id != '' $specific_where";
		$result1 = db_query( $query1 );
		$t_total_reported1 = db_num_rows( $result1 );
		$result2 = db_query( $query2 );
		$t_total_reported2 = db_num_rows( $result2 );
		$t_interviewer_arr1 = array();
		$t_interviewer_arr2 = array();

		for ( $i = 0; $i < $t_total_reported1; $i++ ) {
			$row = db_fetch_array( $result1 );

			if ( isset( $t_interviewer_arr1[$row['fixed_in_version']] ) ) {
				$t_interviewer_arr1[$row['fixed_in_version']]++;
			} else {
				$t_interviewer_arr1[$row['fixed_in_version']] = 1;
			}
		}
		for ( $i = 0; $i < $t_total_reported2; $i++ ) {
			$row = db_fetch_array( $result2 );
			if ( isset( $t_interviewer_arr2[$row['target_version']] ) ) {
				$t_interviewer_arr2[$row['target_version']]++;
			} else {
				$t_interviewer_arr2[$row['target_version']] = 1;
			}
		}

		if ( count( $t_interviewer_arr1 ) == 0 or count( $t_interviewer_arr2 ) == 0) {
			return array();
		}
		var_dump($t_interviewer_arr1);
		echo "-------------------";
		$t_imploded_interviewers1 = implode( '\',\'', array_keys( $t_interviewer_arr1 ) );
		$t_imploded_interviewers2 = implode( '\',\'', array_keys( $t_interviewer_arr2 ) );
		$query1 = "SELECT id,version 
				FROM $t_user_table
				WHERE version IN ('$t_imploded_interviewers')
				ORDER BY version";
		$result = db_query( $query );
		...
		for ($i=0;$i<$user_count;$i++) {
			...
			#$t_metrics[$v_id] = $t_r[$v_version];
			$t_metrics[$v_version] = $t_total_interviews[$v_version];
		} # end for
		return $t_metrics;
	}
	...
	function create_category_summary() {
		global $category_name, $category_candidate_count;

		$t_project_id = helper_get_current_project();
		$t_cat_table = config_get( 'cosmos_project_category_table' );
		$t_candidate_table = config_get( 'cosmos_candidate_table' );
		$t_user_id = auth_get_current_user_id();
		$specific_where = helper_project_specific_where( $t_project_id, $t_user_id );
	...
	function create_cumulative_bydate(){
		...
		#    also, get the last updated date for the candidate as this may be all the information we have
		$query = "SELECT $t_candidate_table.id, last_updated, date_modified, new_value, old_value
			FROM $t_candidate_table LEFT JOIN $t_history_table
			ON $t_candidate_table.id = $t_history_table.candidate_id
			WHERE $specific_where
						AND $t_candidate_table.status >= '$t_res_val'
						AND ( ( $t_history_table.new_value >= '$t_res_val'
								AND $t_history_table.field_name = 'status' )
						OR $t_history_table.id is NULL )
			ORDER BY $t_candidate_table.id, date_modified ASC";
		$result = db_query( $query );
		$candidate_count = db_num_rows( $result );
		...
		for ($i=0;$i<$candidate_count;$i++) {
			...
			#  (i.e. pre 0.18 data), use last_updated from candidate table instead
	...
	#
	# ----------------------------------------------------
	function error_check( $candidate_count, $title ) {
