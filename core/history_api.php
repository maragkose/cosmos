...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	function history_log_event_direct( $p_candidate_id, $p_field_name, $p_old_value, $p_new_value, $p_user_id = null ) {
		...
		if ( $p_new_value != $p_old_value ) {
			...
			$query = "INSERT INTO $t_cosmos_candidate_history_table
						( user_id, candidate_id, date_modified, field_name, old_value, new_value )
					VALUES
						( '$c_user_id', '$c_candidate_id', " . db_now() . ", '$c_field_name', '$c_old_value', '$c_new_value' )";
	...
	# log the changes
	# events should be logged *after* the modification
	function history_log_event( $p_candidate_id, $p_field_name, $p_old_value ) {
		history_log_event_direct( $p_candidate_id, $p_field_name, $p_old_value, candidate_get_field( $p_candidate_id, $p_field_name ) );
	}
	# --------------------
	# log the changes
	# events should be logged *after* the modification
	# These are special case logs (new candidate, deleted candidatenote, etc.)
	function history_log_event_special( $p_candidate_id, $p_type, $p_optional='',  $p_optional2='' ) {
		$c_candidate_id		= db_prepare_int( $p_candidate_id );
		...
		$query = "INSERT INTO $t_cosmos_candidate_history_table
					( user_id, candidate_id, date_modified, type, old_value, new_value, field_name )
				VALUES
					( '$t_user_id', '$c_candidate_id', " . db_now() . ", '$c_type', '$c_optional', '$c_optional2', '' )";
		$result = db_query( $query );
	}
	# --------------------
	# return all candidate history for a given candidate id ordered by date
	function history_get_events( $p_candidate_id ) {
		$t_cosmos_candidate_history_table	= config_get( 'cosmos_candidate_history_table' );
		...
		$query = "SELECT b.*, u.username
				FROM $t_candidate_history_table b
				LEFT JOIN $t_cosmos_user_table u
				ON b.user_id=u.id
	...
	# --------------------
	# Retrieves the history events for the specified candidate id and returns it in an array
	# The array is indexed from 0 to N-1.  The second dimension is: 'date', 'username',
	# 'note', 'change'.
	function history_get_events_array( $p_candidate_id, $p_user_id = null ) {
		$t_normal_date_format = config_get( 'normal_date_format' );

		$raw_history = history_get_raw_events_array( $p_candidate_id, $p_user_id );
	...
	# --------------------
	# Retrieves the raw history events for the specified candidate id and returns it in an array
	# The array is indexed from 0 to N-1.  The second dimension is: 'date', 'userid', 'username',
	# 'field','type','old_value','new_value'
	function history_get_raw_events_array( $p_candidate_id, $p_user_id = null ) {
		$t_cosmos_candidate_history_table	= config_get( 'cosmos_candidate_history_table' );
		$t_cosmos_user_table		= config_get( 'cosmos_user_table' );
		...
		# I give you an example. We create a child of a candidate with different custom fields. In the history of the child
		# candidate we will find the line related to the relationship mixed with the custom fields (the history is creted
		# for the new candidate with the same timestamp...)
		$query = "SELECT *
		...
		$t_private_candidatenote_visible = access_has_candidate_level( 
			config_get( 'private_candidatenote_threshold' ), $p_candidate_id, $t_user_id );
		...
		for ( $i=0,$j=0; $i < $raw_history_count; ++$i ) {
			...
			if ( false !== $t_field_id && 
					!custom_field_has_read_access( $t_field_id, $p_candidate_id, $t_user_id ) ) {
				continue; 
			}
			
			if ( ( $v_field_name == 'target_version' ) && !access_has_candidate_level( $t_roadmap_view_access_level, $p_candidate_id, $t_user_id ) ) {
				continue;
			}
			...
			if ( $t_user_id != $v_user_id ) { // bypass if user originated note
				if ( ( $v_type == BUGNOTE_ADDED ) ||
					...
					( $v_type == BUGNOTE_DELETED ) ) {
						if ( !$t_private_candidatenote_visible && 
							( candidatenote_get_field( $v_old_value, 'view_state' ) == VS_PRIVATE ) ) {
								continue;
						}
				}
				...
				if ( $v_type == BUGNOTE_STATE_CHANGED ) {
					if ( !$t_private_candidatenote_visible && 
							( candidatenote_get_field( $v_new_value, 'view_state' ) == VS_PRIVATE ) ) {
						continue;
					}
				}
			}
	...
	function history_localize_item( $p_field_name, $p_type, $p_old_value, $p_new_value ) {
		...
		if ( NORMAL_TYPE != $p_type ) {
			switch ( $p_type ) {
				case NEW_BUG:
					$t_note = lang_get( 'new_candidate' );
					break;
				case BUGNOTE_ADDED:
					$t_note = lang_get( 'candidatenote_added' ) . ": " . $p_old_value;
					break;
				case BUGNOTE_UPDATED:
					$t_note = lang_get( 'candidatenote_edited' ) . ": " . $p_old_value;
					break;
				case BUGNOTE_DELETED:
					$t_note = lang_get( 'candidatenote_deleted' ) . ": " . $p_old_value;
					break;
				...
				case BUGNOTE_STATE_CHANGED:
					$p_old_value = get_enum_element( 'view_state', $p_old_value );
					$t_note = lang_get( 'candidatenote_view_state' ) . ": " . $p_old_value . ": " . $p_new_value;
					break;
				case BUG_MONITOR:
					$p_old_value = user_get_name( $p_old_value );
					$t_note = lang_get( 'candidate_monitor' ) . ": " . $p_old_value;
					break;
				case BUG_UNMONITOR:
					$p_old_value = user_get_name( $p_old_value );
					$t_note = lang_get( 'candidate_end_monitor' ) . ": " . $p_old_value;
					break;
				case BUG_DELETED:
					$t_note = lang_get( 'candidate_deleted' ) . ": " . $p_old_value;
					break;
				...
				case BUG_ADD_RELATIONSHIP:
					...
					$t_change = relationship_get_description_for_history( $p_old_value ) . ' ' . candidate_format_id( $p_new_value );
					break;
				...
				case BUG_DEL_RELATIONSHIP:
					...
					# Fix for #7846: There are some cases where old value is empty, this may be due to an old candidate.
					if ( !is_blank( $p_old_value ) && $p_old_value > 0 ) {
						$t_change = relationship_get_description_for_history( $p_old_value ) . ' ' . candidate_format_id( $p_new_value );
					} else {
						$t_change = candidate_format_id( $p_new_value );
					}
					break;
				case BUG_CLONED_TO:
					$t_note = lang_get( 'candidate_cloned_to' );
					$t_change = candidate_format_id( $p_new_value );
					break;
				case BUG_CREATED_FROM:
					$t_note = lang_get( 'candidate_created_from' );
					$t_change = candidate_format_id( $p_new_value );
					break;
				case CHECKIN:
	...
	# --------------------
	# delete all history associated with a candidate
	function history_delete( $p_candidate_id ) {
		$c_candidate_id = db_prepare_int( $p_candidate_id );

		$t_candidate_history_table = config_get( 'cosmos_candidate_history_table' );

		$query = "DELETE FROM $t_candidate_history_table
				  WHERE candidate_id='$c_candidate_id'";
		db_query($query);
