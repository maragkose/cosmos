...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------------------------------------------
	# $Id: print_api.php,v 1.1 2009/01/08 07:46:35 chirag Exp $
	# --------------------------------------------------------
	...
	# --------------------
	# Print a redirect header to view a candidate
	function print_header_redirect_view( $p_candidate_id ) {
		print_header_redirect( string_get_candidate_view_url( $p_candidate_id ) );
	}

	# --------------------
	# Get a view URL for the candidate id based on the user's preference and
	#  call print_successful_redirect() with that URL
	function print_successful_redirect_to_candidate( $p_candidate_id ) {
		$t_url = string_get_candidate_view_url( $p_candidate_id, auth_get_current_user_id() );
	...
	# --------------------
	# Print a redirect header to update a candidate
	function print_header_redirect_update( $p_candidate_id ) {
		print_header_redirect( string_get_candidate_update_url( $p_candidate_id ) );
	}
	# --------------------
	# Print a redirect header to update a candidate
	function print_header_redirect_report() {
		print_header_redirect( string_get_candidate_report_url() );
	}
	...
	# --------------------
	# same as print_user() but fills in the subject with the candidate summary
	function print_user_with_subject( $p_user_id, $p_candidate_id ) {
		...
		if ( user_exists( $p_user_id ) && user_get_field( $p_user_id, 'enabled' ) ) {
			...
			print_email_link_with_subject( $t_email, $t_username, $p_candidate_id );
		} else {
	...
	function print_duplicate_id( $p_duplicate_id ) {
		if ( $p_duplicate_id != 0 ) {
			PRINT string_get_candidate_view_link( $p_duplicate_id );
		}
	}
	# --------------------
	...
	function print_user_option_list( $p_user_id, $p_project_id = null, $p_access = ANYBODY ) {
		...
		for ($i = 0; $i < count( $t_sort ); $i++ ) {
			...
			PRINT '>' . $t_display[$i] ." (".get_enum_element( 'access_levels', user_get_access_level($t_row['id'],$p_project_id)). ') </option>';
		}
	}
//echo  ; 
	# --------------------
	...
	#  actually reported the candidates at the time. Maybe we could get all user
	#  who are listed as the reporter in any candidate?  It would probably be a
	#  faster query actually.
	function print_reporter_option_list( $p_user_id, $p_project_id = null ) {
		print_user_option_list( $p_user_id, $p_project_id, config_get( 'report_candidate_threshold' ) );
	}

	# --------------------
	function print_duplicate_id_option_list() {
	    $query = "SELECT id
	    		FROM " . config_get ( 'cosmos_candidate_table' ) . "
	    		ORDER BY id ASC";
	...
	/**
	 * Print the entire form for attaching a tag to a candidate.
	 * @param integer Bug ID
	 * @param string Default contents of the input box
	 */
	function print_tag_attach_form( $p_candidate_id, $p_string="" ) {
		?>
		...
		<input type="hidden" name="candidate_id" value="<?php echo $p_candidate_id ?>" />
		<?php
			print_tag_input( $p_candidate_id, $p_string );
		?>
	 ...
	 * @param string Default contents of the input box
	 */
	function print_tag_input( $p_candidate_id = 0, $p_string="" ) {
		?>
		...
		<select <?php echo helper_get_tab_index() ?> name="tag_select" id="tag_select" onchange="tag_string_append( this.options[ this.selectedIndex ].text );">
			<?php print_tag_option_list( $p_candidate_id ); ?>
		</select>
		<?php
	 ...
	 * When passed a candidate ID, the option list will not contain any tags attached to the given candidate.
	 * @param integer Bug ID
	 */
	function print_tag_option_list( $p_candidate_id = 0 ) {
		$t_tag_table = config_get( 'cosmos_tag_table' );
		...
		if ( 0 != $p_candidate_id ) {
			...
			$query .= "	WHERE id NOT IN ( 
						SELECT tag_id FROM $t_candidate_tag_table WHERE candidate_id='$c_candidate_id' ) ";
		}
	...
	function print_field_option_list( $p_list, $p_item='' ) {
		...
		$t_category_string = get_enum_string( $t_cosmos_candidate_table, $p_list );
	...
	function print_assign_to_option_list( $p_user_id='', $p_project_id = null, $p_threshold = null ) {
		...
		if ( null === $p_threshold ) {
			$p_threshold = config_get( 'handle_candidate_threshold' );
		}
		//Modifed by Chirag A, we allow assign only for recruiter or higer role, not available for candiate
		print_user_option_list( $p_user_id, $p_project_id, '40');
	}
	# --------------------
	...
	function print_news_project_option_list( $p_project_id ) {
		...
		$t_cosmos_project_user_list_table = config_get( 'cosmos_project_user_list_table' );

		if ( access_has_project_level( ADMINISTRATOR ) ) {
			$query = "SELECT *
					FROM $t_cosmos_project_table
					ORDER BY name";
		} else {
			$t_user_id = auth_get_current_user_id();
			$query = "SELECT p.id, p.name
					FROM $t_cosmos_project_table p, $t_cosmos_project_user_list_table m
	...
	# We check in the project category table and in the candidate table
	...
	function print_category_option_list( $p_category='', $p_project_id = null ) {
		$t_cosmos_project_category_table = config_get( 'cosmos_project_category_table' );
		...
		$query = "SELECT DISTINCT category
				FROM $t_cosmos_project_category_table
	...
	function print_category_complete_option_list( $p_category='', $p_project_id = null ) {
		...
		# grab all categories in the candidate table
		$query = "SELECT DISTINCT category
...
#
	...
	#	}
		...
		print_user_option_list( '', $p_project_id, '40');
	}
	# --------------------
	...
	function get_status_option_list( $p_user_auth = 0, $p_current_value = 0, $p_show_current = true, $p_add_close = false ) {
		...
		if ( ( true == $p_add_close ) && 
					( access_compare_level( $p_current_value, config_get( 'candidate_resolved_status_threshold' ) ) ) ) {
				$t_enum_list[CLOSED] = get_enum_element( 'status', CLOSED );
			}
		return $t_enum_list;
	}
	# --------------------
	# print the status option list for the candidate_update pages
	...
	# --------------------
	# @@@ preliminary support for multiple candidate actions.
	function print_all_candidate_action_option_list() {
		...
		if ( ALL_PROJECTS != $t_project_id ) {
			...
			if ( access_has_project_level( config_get( 'update_candidate_threshold' ), $t_project_id ) ) {
	...
	function print_project_user_list_option_list( $p_project_id=null ) {
		...
		$query = "SELECT DISTINCT u.id, u.username, u.realname
				FROM $t_cosmos_user_table u
				LEFT JOIN $t_cosmos_project_user_list_table p
	...
	function print_project_user_list_option_list2( $p_user_id ) {
		...
		$query = "SELECT DISTINCT p.id, p.name
				FROM $t_cosmos_project_table p
				LEFT JOIN $t_cosmos_project_user_list_table u
	...
	###########################################################################
	# --------------------
	# prints a link to VIEW a candidate given an ID
	#  account for the user preference and site override
	function print_candidate_link_home( $p_candidate_id, $p_detail_info = true ) {
		PRINT string_get_candidate_view_link( $p_candidate_id, null, $p_detail_info );
	}
	function print_candidate_link( $p_candidate_id, $name, $p_detail_info = true ) {
		PRINT string_get_candidate_view_link( $p_candidate_id, $name, null, $p_detail_info );
	}

	# --------------------
	# prints a link to UPDATE a candidate given an ID
	#  account for the user preference and site override
	function print_candidate_update_link( $p_candidate_id ) {
		PRINT string_get_candidate_update_link( $p_candidate_id );
	}

 	# --------------------
	# formats the priority given the status
	# shows the priority in BOLD if the candidate is NOT closed and is of significant priority
	...
	# shows the severity in BOLD if the candidate is NOT closed and is of significant severity
	...
	function print_project_version_string( $p_project_id ) {
		$t_cosmos_project_version_table = config_get( 'cosmos_project_version_table' );
		$t_cosmos_project_table = config_get( 'cosmos_project_table' );

		$c_project_id = db_prepare_int( $p_project_id );

		$query = "SELECT version
				FROM $t_cosmos_project_version_table
				WHERE project_id='$c_project_id'";
		$result = db_query( $query );
	...
	function print_view_candidate_sort_link( $p_string, $p_sort_field, $p_sort, $p_dir, $p_columns_target = COLUMNS_TARGET_VIEW_PAGE ) {
		if ( $p_columns_target == COLUMNS_TARGET_PRINT_PAGE ) {
			...
			echo '<a href="view_all_set.php?sort='.$p_sort_field.'&amp;dir='.$p_dir.'&amp;type=1&amp;print=1">'.$p_string.'</a>';
			...
			echo '<a href="view_all_set.php?sort='.$p_sort_field.'&amp;dir='.$p_dir.'&amp;type=1">'.$p_string.'</a>';
		} else {
			echo $p_string;
		}
	}
	...
	function print_button( $p_action_page, $p_label, $p_args_to_post = null ) {
		echo '<form action="', $p_action_page,'" method="post">';
	...
	#function print_bracket_link( $p_link, $p_button_text, $p_new_window = false ) {
	#	PRINT "<form><input class=\"button-small\" type=\"button\" value=\"$p_button_text\" onClick=\"window.location.href='$p_link'\"></form>";  
	#}
	function print_button_link( $p_link, $p_button_text, $p_class, $p_new_window = false ) {
		if( true == $p_new_window ) {
			PRINT "<a href=\"$p_link\" target=\"_blank\"><input type=\"button\" class=\"$p_class\" value=\"$p_button_text\"/></a>";
		} else {
			PRINT "<a href=\"$p_link\"><input type=\"button\" class=\"$p_class\" value=\"$p_button_text\"/></a>";
		}
	}
	...
	function print_email_link_with_subject( $p_email, $p_text, $p_candidate_id ) {
	...
	function print_recently_visited() {
		...
		foreach( $t_ids as $t_id ) {
			...
			echo string_get_candidate_view_link( $t_id );
		}
		echo '</small></div>';
	}
