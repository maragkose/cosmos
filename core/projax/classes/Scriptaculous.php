...
function in_place_editor($field_id,$options,$tag=true) {
	...
	function text_field_with_auto_complete($object,$tag_options=null,$completion_options=null)
	{
		...
		# @@@ Fixed a candidate in the next like where the dumping of the value tag used to check if size is set!
		...
		$ret_val.='<input id="'.$object.'" name="'.$object.'"'. $t_tabindex . $t_maxlength . ' size="'.(isset($tag_options['size'])?$tag_options['size']:30).'" type="text" value="'.(isset($tag_options['value'])?$tag_options['value']:'').'" '.(isset($tag_options['class'])?'class = "'.$tag_options['class'].'" ':'').'/>';
