...
/**
 * Abstract interface for a COSMOS session handler.
 */
/* abstract */ class COSMOSSession {
	var $id;
...
/**
 * Implementation of the abstract COSMOS session interface using
 * standard PHP sessions stored on the server's filesystem according
 * to PHP's session.* settings in 'php.ini'.
 */
class COSMOSPHPSession extends COSMOSSession {
...
function session_init( $p_session_id=null ) {
	...
	switch( strtolower( $g_session_handler ) ) {
		case 'php':
			$g_session = new COSMOSPHPSession( $p_session_id );
			break;
