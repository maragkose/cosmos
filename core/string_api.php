...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------------------------------------------
	# $Id: string_api.php,v 1.1 2009/01/08 07:46:35 chirag Exp $
	# --------------------------------------------------------
	...
	#  candidate references, and cvs references
	function string_display_links( $p_string ) {
		$p_string = string_display( $p_string );
		$p_string = string_insert_hrefs( $p_string );
		$p_string = string_process_candidate_link( $p_string );
		$p_string = string_process_candidatenote_link( $p_string );
	...
	# URLs, emails, candidate references, and cvs references
	...
	function string_rss_links( $p_string ) {
		...
		$t_string = string_process_candidate_link( $t_string, /* anchor */ true, /* detailInfo */ false, /* fqdn */ true );
		$t_string = string_process_candidatenote_link( $t_string, /* anchor */ true, /* detailInfo */ false, /* fqdn */ true );
	...
	# --------------------
	# Prepare a string for plain text display in email and add URLs for candidate
	#  links and cvs links
	function string_email_links( $p_string ) {
		$p_string = string_email( $p_string );
		$p_string = string_process_candidate_link( $p_string, false );
		$p_string = string_process_candidatenote_link( $p_string, false );
	...
	# --------------------
	# Process $p_string, looking for candidate ID references and creating candidate view
	#  links for them.
	#
	...
	#  the URL
	#
	# The candidate tag ('#' by default) must be at the beginning of the string or
	...
	$string_process_candidate_link_callback = array();

	function string_process_candidate_link( $p_string, $p_include_anchor = true, $p_detail_info = true, $p_fqdn = false ) {
		global $string_process_candidate_link_callback;

		$t_tag = config_get( 'candidate_link_tag' );
		# bail if the link tag is blank
		if ( '' == $t_tag || $p_string == '' ) {
			return $p_string;
		}

		if ( !isset( $string_process_candidate_link_callback[$p_include_anchor][$p_detail_info][$p_fqdn] ) ) {
			if ($p_include_anchor) {
				$string_process_candidate_link_callback[$p_include_anchor][$p_detail_info][$p_fqdn] = create_function('$p_array','
										if ( candidate_exists( (int)$p_array[2] ) && access_has_candidate_level( VIEWER, (int)$p_array[2] ) ) {
											return $p_array[1] . string_get_candidate_view_link( (int)$p_array[2], null, ' . ($p_detail_info ? 'true' : 'false') . ', ' . ($p_fqdn ? 'true' : 'false') . ');
										} else {    	
											return $p_array[0];
										}
										');
				...
				$string_process_candidate_link_callback[$p_include_anchor][$p_detail_info][$p_fqdn] = create_function('$p_array','
										# We might as well create the link here even if the candidate
										#  doesnt exist.  In the case above we dont want to do
										#  the summary lookup on a non-existant candidate.  But here, we
										#  can create the link and by the time it is clicked on, the
										#  candidate may exist.			
										return $p_array[1] . string_get_candidate_view_url_with_fqdn( (int)$p_array[2], null );
										');
			}
		}

		$p_string = preg_replace_callback( '/(^|[^\w&])' . preg_quote($t_tag, '/') . '(\d+)\b/', $string_process_candidate_link_callback[$p_include_anchor][$p_detail_info][$p_fqdn], $p_string);
		return $p_string;
	}

	# --------------------
	# Process $p_string, looking for candidatenote ID references and creating candidate view
	#  links for them.
	#
	# Returns the processed string.
	#
	...
	#  the URL
	#
	# The candidatenote tag ('~' by default) must be at the beginning of the string or
	#  preceeded by a character that is not a letter, a number or an underscore
	#
	# if $p_include_anchor = false, $p_fqdn is ignored and assumed to true.
	$string_process_candidatenote_link_callback = array();

	function string_process_candidatenote_link( $p_string, $p_include_anchor = true, $p_detail_info = true, $p_fqdn = false ) {
		global $string_process_candidatenote_link_callback;
		$t_tag = config_get( 'candidatenote_link_tag' );
		...
		if ( !isset ( $string_process_candidatenote_link_callback[$p_include_anchor][$p_detail_info][$p_fqdn] ) ) {
			if ($p_include_anchor) {
				$string_process_candidatenote_link_callback[$p_include_anchor][$p_detail_info][$p_fqdn] = create_function('$p_array','
										if ( candidatenote_exists( (int)$p_array[2] ) ) {
											$t_candidate_id = candidatenote_get_field( (int)$p_array[2], \'candidate_id\' );
											if ( candidate_exists( $t_candidate_id ) ) {
												return $p_array[1] . string_get_candidatenote_view_link( $t_candidate_id, (int)$p_array[2], null, ' . ($p_detail_info ? 'true' : 'false') . ', ' . ($p_fqdn ? 'true' : 'false') . ' );
											} else {
												return $p_array[0];
											}
										} else {
				...
				$string_process_candidatenote_link_callback[$p_include_anchor][$p_detail_info][$p_fqdn] = create_function('$p_array','
										...
										$t_candidate_id = candidatenote_get_field( (int)$p_array[2], \'candidate_id\' );
										if ( candidate_exists( $t_candidate_id ) ) {
											return $p_array[1] . string_get_candidatenote_view_url_with_fqdn( $t_candidate_id, (int)$p_array[2], null );
										} else {
											return $p_array[0];
										}
										');
		...
		$p_string = preg_replace_callback( '/(^|[^\w])' . preg_quote($t_tag, '/') .'(\d+)\b/', $string_process_candidatenote_link_callback[$p_include_anchor][$p_detail_info][$p_fqdn], $p_string);
		return $p_string;
	}
	...
	# --------------------
	# return the name of a candidate page for the user
	...
	function string_get_candidate_page( $p_action, $p_user_id=null ) {
		if ( null === $p_user_id ) {
		...
		switch ( $g_show_action ) {
			case BOTH:
					if ( ( null !== $p_user_id ) &&
						...
						return 'candidate_' . $p_action . '_advanced_page.php';
					} else {
						return 'candidate_' . $p_action . '_page.php';
					}
			case SIMPLE_ONLY:
					return 'candidate_' . $p_action . '_page.php';
			case ADVANCED_ONLY:
	...
	# --------------------
	# return an href anchor that links to a candidate VIEW page for the given candidate
	#  account for the user preference and site override
	function string_get_candidate_view_link( $p_candidate_id, $name, $p_user_id = null, $p_detail_info = true, $p_fqdn = false ) {
		if ( candidate_exists( $p_candidate_id ) ) {
			$t_link = '<a href="';
			if ( $p_fqdn ) {
				$t_link .= config_get( 'path' );
			}
			$t_link .= string_get_candidate_view_url( $p_candidate_id, $p_user_id ) . '"';
			if ( $p_detail_info ) {
				$t_summary = string_attribute( candidate_get_field( $p_candidate_id, 'summary' ) );
				$t_status = string_attribute( get_enum_element( 'status', candidate_get_field( $p_candidate_id, 'status' ) ) );
				$t_link .=  ' title="[' . $t_status . '] ' . $t_summary . '"';
			}
			$t_link .= '>' . $name . '</a>';
		} else {
			$t_link = candidate_format_id( $p_candidate_id );
		}
	...
	# return an href anchor that links to a candidate VIEW page for the given candidate
	#  account for the user preference and site override
	function string_get_candidatenote_view_link( $p_candidate_id, $p_candidatenote_id, $p_user_id = null, $p_detail_info = true, $p_fqdn = false ) {
		if ( candidate_exists( $p_candidate_id ) && candidatenote_exists( $p_candidatenote_id ) ) {
			$t_link = '<a href="';
			if ( $p_fqdn ) {
				$t_link .= config_get( 'path' );
			}

			$t_link .= string_get_candidatenote_view_url( $p_candidate_id, $p_candidatenote_id, $p_user_id ) . '"';
			if ( $p_detail_info ) {
				$t_reporter = string_attribute( user_get_name ( candidatenote_get_field( $p_candidatenote_id, 'reporter_id' ) ) );
				$t_update_date = string_attribute( date( config_get( 'normal_date_format' ), ( db_unixtimestamp( candidatenote_get_field( $p_candidatenote_id, 'last_modified' ) ) ) ) );
				$t_link .=  ' title="[' . $t_update_date . '] ' . $t_reporter . '"';
			}
			$t_link .= '>' . lang_get( 'candidatenote' ) . ': ' . candidatenote_format_id( $p_candidatenote_id) . '</a>';
		} else {
			$t_link = lang_get( 'candidatenote' ) . ': ' . candidatenote_format_id( $p_candidatenote_id);
		}
	...
	# --------------------
	# return the name and GET parameters of a candidate VIEW page for the given candidate
	#  account for the user preference and site override
	function string_get_candidate_view_url( $p_candidate_id, $p_user_id = null ) {
		return 'view.php?id=' . $p_candidate_id;
	}

	# --------------------
	# return the name and GET parameters of a candidate VIEW page for the given candidate
	#  account for the user preference and site override
	function string_get_candidatenote_view_url( $p_candidate_id, $p_candidatenote_id, $p_user_id = null ) {
		return 'view.php?id=' . $p_candidate_id . '#c'. $p_candidatenote_id;
	}
	...
	# in emails.
	function string_get_candidatenote_view_url_with_fqdn( $p_candidate_id, $p_candidatenote_id, $p_user_id = null ) {
		return config_get( 'path' ) . string_get_candidate_view_url( $p_candidate_id, $p_user_id ).'#c'.$p_candidatenote_id;
	}
	...
	# The returned url includes the fully qualified domain, hence it is suitable to be included
	# in emails.
	function string_get_candidate_view_url_with_fqdn( $p_candidate_id, $p_user_id = null ) {
		return config_get( 'path' ) . string_get_candidate_view_url( $p_candidate_id, $p_user_id );
	}

	# --------------------
	# return the name of a candidate VIEW page for the user
	#  account for the user preference and site override
	function string_get_candidate_view_page( $p_user_id = null ) {
		return string_get_candidate_page( 'view', $p_user_id );
	}

	# --------------------
	# return an href anchor that links to a candidate UPDATE page for the given candidate
	#  account for the user preference and site override
	function string_get_candidate_update_link( $p_candidate_id, $p_user_id = null ) {
		$t_summary = string_attribute( candidate_get_field( $p_candidate_id, 'summary' ) );
		return '<a href="' . string_get_candidate_update_url( $p_candidate_id, $p_user_id ) . '" title="' . $t_summary . '">' . candidate_format_id( $p_candidate_id ) . '</a>';
	}

	# --------------------
	# return the name and GET parameters of a candidate UPDATE page for the given candidate
	#  account for the user preference and site override
	function string_get_candidate_update_url( $p_candidate_id, $p_user_id = null ) {
		return string_get_candidate_update_page( $p_user_id ) . '?candidate_id=' . $p_candidate_id;
	}

	# --------------------
	# return the name of a candidate UPDATE page for the user
	#  account for the user preference and site override
	function string_get_candidate_update_page( $p_user_id = null ) {
		return string_get_candidate_page( 'update', $p_user_id );
	}

	# --------------------
	# return an href anchor that links to a candidate REPORT page for the given candidate
	#  account for the user preference and site override
	function string_get_candidate_report_link( $p_user_id = null ) {
		return '<a href="' . string_get_candidate_report_url( $p_user_id ) . '">' . lang_get( 'report_candidate_link' ) . '</a>';
	}

	# --------------------
	# return the name and GET parameters of a candidate REPORT page for the given candidate
	#  account for the user preference and site override
	function string_get_candidate_report_url( $p_user_id = null ) {
		return string_get_candidate_report_page( $p_user_id );
	}

	# --------------------
	# return the name of a candidate REPORT page for the user
	#  account for the user preference and site override
	function string_get_candidate_report_page( $p_user_id = null ) {
		return string_get_candidate_page( 'report', $p_user_id );
	}
