...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------------------------------------------
	# $Id: summary_api_ofc.php,v 1.1 2009/01/08 07:46:35 chirag Exp $
	# --------------------------------------------------------
	...
	Global $ChartData;
		...
		array_push($ChartData['label'],$p_label);
		array_push($ChartData['open'],$p_open);
		array_push($ChartData['resolved'],$p_resolved);
		array_push($ChartData['closed'],$p_closed);
		array_push($ChartData['total'],$p_total);
		}

	}
	
	# Added By Chirag A for OFC
	function summary_helper_print_row_ofc( $p_label, $p_open, $p_resolved, $p_closed, $p_total ) {
	//	
		Global $ChartData;
		array_push($ChartData['label'],$p_label);
		array_push($ChartData['open'],$p_open);
...
#Added By Chirag A to show Flash chart
	# --------------------
	# Used in summary reports
	# Given the enum string this function prints out the summary
	# for each enum setting
	# The enum field name is passed in through $p_enum
	function summary_print_by_enum_ofc( $p_enum_string, $p_enum ) {
		$t_project_id = helper_get_current_project();
		$t_user_id = auth_get_current_user_id();
		...
		if ( 0 < $t_candidates_total ) {
			...
			summary_helper_print_row_ofc( get_enum_element( $p_enum, $t_last_value), $t_candidates_open, $t_candidates_resolved, $t_candidates_closed, $t_candidates_total );
		}
	}


#End of code added by chirag#
	...
	Global $PieChartData;
		...
		while ( $row = db_fetch_array( $result ) ) {
			...
			$t_candidateid = string_get_candidate_view_url( $row['id'] );
	...
	//	echo  $p_label."|".$p_open."|".$p_resolved."|".$p_closed."|".$p_total."<BR>" ;
			if(is_array($PieChartData)){
			array_push($PieChartData['labels'],$t_summary);
			array_push($PieChartData['links'],$t_candidateid);
			array_push($PieChartData['data'],$t_notescount);
			}
	...
	function summary_print_by_age() {
		...
		while ( $row = db_fetch_array( $result ) ) {
			...
			array_push($PieChartData['labels'],$t_summary);
			array_push($PieChartData['links'],$t_candidateid);
			array_push($PieChartData['data'],$t_days_open);
			}
//			print "<tr " . helper_alternate_class() . ">\n";
