...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	function tag_update( $p_tag_id, $p_name, $p_user_id, $p_description ) {
		...
		if ( $t_rename ) {
			$t_candidates = tag_get_candidates_attached( $p_tag_id );

			foreach ( $t_candidates as $t_candidate_id ) {
				history_log_event_special( $t_candidate_id, TAG_RENAMED, $t_tag_name, $c_name );
			}
		}
	...
	function tag_delete( $p_tag_id ) {
		...
		$t_candidates = tag_get_candidates_attached( $p_tag_id );
		foreach ( $t_candidates as $t_candidate_id ) {
			tag_candidate_detach( $p_tag_id, $t_candidate_id );
		}
	...
	/**
	 * Determine if a tag is attached to a candidate.
	 * @param integer Tag ID
	 * @param integer Bug ID
	 * @return boolean True if the tag is attached
	 */
	function tag_candidate_is_attached( $p_tag_id, $p_candidate_id ) {
		$c_tag_id 		= db_prepare_int( $p_tag_id );
		$c_candidate_id 		= db_prepare_int( $p_candidate_id );

		$t_candidate_tag_table= config_get( 'cosmos_candidate_tag_table' );

		$query = "SELECT * FROM $t_candidate_tag_table
					WHERE tag_id='$c_tag_id' AND candidate_id='$c_candidate_id'";
		$result = db_query( $query );
		return ( db_num_rows( $result ) > 0 );
	}
	 ...
	 */
	function tag_candidate_get_row( $p_tag_id, $p_candidate_id ) {
		$c_tag_id 		= db_prepare_int( $p_tag_id );
		$c_candidate_id 		= db_prepare_int( $p_candidate_id );
	...
	/**
	 * Return an array of tags attached to a given candidate sorted by tag name.
	 * @param Bug ID
	 * @return array Array of tag rows with attachement information
	 */
	function tag_candidate_get_attached( $p_candidate_id ) {
		$c_candidate_id 		= db_prepare_int( $p_candidate_id );
		...
		$query = "SELECT t.*, b.user_id as user_attached, b.date_attached
					...
					LEFT JOIN $t_candidate_tag_table as b
						on t.id=b.tag_id
	...
	/**
	 * Return an array of candidates that a tag is attached to.
	 * @param integer Tag ID
	 * @return array Array of candidate ID's.
	 */
	function tag_get_candidates_attached( $p_tag_id ) {
		$c_tag_id 		= db_prepare_int( $p_tag_id );

		$t_candidate_tag_table= config_get( 'cosmos_candidate_tag_table' );

		$query = "SELECT candidate_id FROM $t_candidate_tag_table
	...
	/**
	 * Attach a tag to a candidate.
	 * @param integer Tag ID
	 * @param integer Bug ID
	 * @param integer User ID
	 */
	function tag_candidate_attach( $p_tag_id, $p_candidate_id, $p_user_id=null ) {
		access_ensure_candidate_level( config_get( 'tag_attach_threshold' ), $p_candidate_id, $p_user_id );

		tag_ensure_exists( $p_tag_id );

		if ( tag_candidate_is_attached( $p_tag_id, $p_candidate_id ) ) {
		...
		$query = "INSERT INTO $t_candidate_tag_table
					( tag_id,
					  candidate_id,
					  user_id,
					  date_attached
		...
		$t_tag_name = tag_get_field( $p_tag_id, 'name' );
		history_log_event_special( $p_candidate_id, TAG_ATTACHED, $t_tag_name );
	 ...
	 * @param integer Tag ID
	 * @param integer Bug ID
	 * @param boolean Add history entries to candidate
	 * @param integer User Id (or null for current logged in user)	 
	 */
	function tag_candidate_detach( $p_tag_id, $p_candidate_id, $p_add_history=true, $p_user_id = null ) {
		if ( $p_user_id === null ) {
		...
		$t_tag_row = tag_candidate_get_row( $p_tag_id, $p_candidate_id);
		...
		access_ensure_candidate_level( $t_detach_level, $p_candidate_id, $t_user_id );
		...
		$query = "DELETE FROM $t_candidate_tag_table 
					WHERE tag_id='$c_tag_id' AND candidate_id='$c_candidate_id'";
		db_query( $query );

		if ( $p_add_history ) {
			$t_tag_name = tag_get_field( $p_tag_id, 'name' );
			history_log_event_special( $p_candidate_id, TAG_DETACHED, $t_tag_name );
		}
	...
	/**
	 * Detach all tags from a given candidate.
	 * @param integer Bug ID
	 * @param boolean Add history entries to candidate
	 * @param integer User Id (or null for current logged in user)	 
	 */
	function tag_candidate_detach_all( $p_candidate_id, $p_add_history=true, $p_user_id = null ) {
		$t_tags = tag_candidate_get_attached( $p_candidate_id );
		foreach ( $t_tags as $t_tag_row ) {
			tag_candidate_detach( $t_tag_row['id'], $p_candidate_id, $p_add_history, $p_user_id );
		}
	}
	 ...
	 * If a candidate ID is passed, the tag link will include a detach link if the 
	 * user has appropriate privileges.
	 * @param array Tag row
	 * @param integer Bug ID
	 */
	function tag_display_link( $p_tag_row, $p_candidate_id=0 ) {
		...
		if( $p_candidate_id > 0 && access_has_candidate_level( $t_detach, $p_candidate_id ) ) {
			$t_tooltip = sprintf( lang_get( 'tag_detach' ), $t_name );
			echo " <a href='tag_detach.php?candidate_id=$p_candidate_id&tag_id=$p_tag_row[id]$t_security_token'><img src='images/delete.png' class='delete-icon' title=\"$t_tooltip\"/></a>";
		}
	 ...
	 * @param Bug ID
	 */
	function tag_display_attached( $p_candidate_id ) {
		$t_tag_rows = tag_candidate_get_attached( $p_candidate_id );
		...
		if ( count( $t_tag_rows ) == 0 ) {
			...
			foreach ( $t_tag_rows as $t_tag ) {
				...
				tag_display_link( $t_tag, $p_candidate_id );
				$i++;
			}
		}
	...
	/**
	 * Get the number of candidates a given tag is attached to.
	 * @param integer Tag ID
	 * @return integer Number of attached candidates
	 */
	function tag_stats_attached( $p_tag_id ) {
		$c_tag_id = db_prepare_int( $p_tag_id );
		$t_candidate_tag_table = config_get( 'cosmos_candidate_tag_table' );

		$query = "SELECT COUNT(*) FROM $t_candidate_tag_table
					WHERE tag_id='$c_tag_id'";
		$result = db_query( $query );
	 ...
	 * based on the number of times they have been attached to the same candidates.
