...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------
	# Posts a twitter update when a candidate is resolved.
	# @param $p_candidate_id The candidate id that was resolved.
	function twitter_issue_resolved( $p_candidate_id ) {
		if ( !twitter_enabled() ) {
			return true;
		}
		
		$t_candidate = candidate_get( $p_candidate_id, false );

		# Do not twitter except fixed issues
		if ( $t_candidate->resolution != FIXED ) {
			return true;
		}

		# Do not twitter private candidates.
		if ( $t_candidate->view_state != VS_PUBLIC ) {
			return true;
		}
		
		# Do not twitter candidates belonging to private projects.
		if ( VS_PRIVATE == project_get_field( $t_candidate->project_id, 'view_state' ) ) {
			return true;
		}

		$c_candidate_id = db_prepare_int( $p_candidate_id );

		if ( is_blank( $t_candidate->fixed_in_version ) ) {
			$t_message = sprintf( 
							lang_get( 'twitter_resolved_no_version' ), 
							$c_candidate_id, 
							$t_candidate->category,
							$t_candidate->summary, 
							user_get_name( $t_candidate->handler_id ) );
		} else {
			$t_message = sprintf(
