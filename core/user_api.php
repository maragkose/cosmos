...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------
	# return whether user is monitoring candidate for the user id and candidate id
	function user_is_monitoring_candidate( $p_user_id, $p_candidate_id ) {
		$c_user_id	= db_prepare_int( $p_user_id );
		...
		$query = "SELECT COUNT(*)
				  ...
				  WHERE user_id='$c_user_id' AND candidate_id='$c_candidate_id'";
...
*/
	...
	function user_delete_profiles( $p_user_id ) {
		...
		$t_user_profile_table = config_get('cosmos_user_profile_table');
	...
	function user_get_field( $p_user_id, $p_field_name ) {
		...
		if ( NO_USER == $p_user_id ) {
			log_event( LOG_EMAIL, "$p_field_name, user_get_field() for NO_USER");
			return '';
		}
	...
	function user_get_email( $p_user_id ) {
		...
		if ($p_user_id == NO_USER || $p_user_id = '') {
			return '';
		}
		return $t_email;
	}
	function user_get_manager_emails() {
		$t_emails = array();
		$t_user_table = config_get( 'cosmos_user_table' );
		$t_manager_level = MANAGER;
		$query = "SELECT email from $t_user_table WHERE access_level >= $t_manager_level AND enabled=1";
		$db_result = db_query( $query );
		$row_count = db_num_rows( $db_result );

		for ( $i=0 ; $i < $row_count ; $i++ ) {
			$row = db_fetch_array( $db_result );
			$t_emails[$i] =  $row['email']; 
		}
	...
	function user_get_interviewer_emails() {
		$t_emails = array();
		$t_user_table = config_get( 'cosmos_user_table' );
		$t_level = DEVELOPER;
		$query = "SELECT email from $t_user_table WHERE access_level = $t_level AND enabled=1";
		$db_result = db_query( $query );
		$row_count = db_num_rows( $db_result );
	...
	# --------------------
	# lookup the user's email in LDAP or the db as appropriate
	function second_inter_get_email( $p_name ) {
		$t_email = '';
		$t_user_table = config_get( 'cosmos_user_table' );
		if ( is_blank( $t_email ) ) {
			$parts = explode (" ",$p_name);
			$query = "SELECT email from $t_user_table WHERE realname like '%$parts[0]%'";
			$db_result = db_query( $query );
	...
	*/
	...
	# --------------------
	# return the number of open assigned candidates to a user in a project
	function user_get_assigned_open_candidate_count( $p_user_id, $p_project_id=ALL_PROJECTS ) {
		$c_user_id		= db_prepare_int($p_user_id);
		$c_project_id	= db_prepare_int($p_project_id);
	...
	# --------------------
	# return the number of open reported candidates by a user in a project
	function user_get_reported_open_candidate_count( $p_user_id, $p_project_id=ALL_PROJECTS ) {
		$c_user_id		= db_prepare_int($p_user_id);
		$c_project_id	= db_prepare_int($p_project_id);
	...
	# --------------------
	# return the candidate filter parameters for the specified user
	function user_get_candidate_filter( $p_user_id, $p_project_id = null ) {
	...
	function user_set_password( $p_user_id, $p_password, $p_allow_protected=false ) {
		...
		$c_user_table	= config_get( 'cosmos_user_table' );
