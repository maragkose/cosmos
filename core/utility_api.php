...
function multi_sort( $p_array, $p_key, $p_direction = ASCENDING ) {
	...
	// Security measure: see http://www.cosmosbt.org/candidates/view.php?id=9704 for details
...
#MANOS
function format_end($p_unix, $p_duration=3600) {
		date_default_timezone_set('UTC');
		$p_unix = strtotime($p_unix);
		$t_unix_d = date("Ymd", $p_unix);
		$t_unix_t = date("Gis", $p_unix + $p_duration );
		$final = str_pad($t_unix_t, 6, '0', STR_PAD_LEFT);
		return $t_unix_d . 'T' .  $final . 'Z';
	}
	function format_start($p_date, $p_time ) {
		if ($p_date == 0 or $p_time == 'Undefined' or $p_time == '' or $p_date == ''){
			return 0;
		}
		date_default_timezone_set('UTC');
		$t_timestamp =  date("Ymd", $p_date);
		if ($p_time != '') {
			$parts = explode(' ', $p_time);
			$parts2 = explode(':', $parts[0]);
			$t = $parts2[0] . $parts2[1];
			$t=$t-300;
			$final = str_pad($t, 4, '0', STR_PAD_LEFT);
		} else {
			return '';
		}
		$t_timestamp .= 'T';
	...
	function get_candidate_hyperlink($candidate_id) {
		$link =  'http://' . $_SERVER['SERVER_NAME'] . dirname($_SERVER['PHP_SELF']) . '/view.php?id=' . $candidate_id;
		return $link;
	}
	# --------------------
