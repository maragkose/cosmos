...
# along with COSMOSBT.  If not, see <http://www.gnu.org/licenses/>.
	...
	require( 'print_all_candidate_options_inc.php' );
	...
	$f_candidate_arr = explode( ',', $f_export );
	...
	do {
		...
		foreach( $result as $t_row ) {
			$t = print_r($t_row, true);
			log_event (LOG_EMAIL, "t_row = $t");
			if ( is_blank( $f_export ) || in_array( $t_row->id, $f_candidate_arr ) ) {
				echo excel_get_start_row();

				foreach ( $t_columns as $t_column ) {
					$t_custom_field = column_get_custom_field_name( $t_column );
					if ( $t_custom_field !== null ) {
					log_event(LOG_EMAIL,  'custom');
					log_event(LOG_EMAIL,  $t_custom_field);
					log_event(LOG_EMAIL,  $t_column);
						echo excel_format_custom_field( $t_row->id, $t_row->project_id, $t_custom_field );
					} else if ( column_is_plugin_column( $t_column ) ) {
					log_event(LOG_EMAIL,  'plug');
					log_event(LOG_EMAIL,  $t_column);
						echo excel_format_plugin_column_value( $t_column, $t_row );
					} else {
					log_event(LOG_EMAIL,  "row =$t_row");
					log_event(LOG_EMAIL,  $t_column);
