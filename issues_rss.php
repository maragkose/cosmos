...
<?php
	...
	$image_link = $t_path . 'images/cosmos_logo_button.gif';
	...
	$t_issues = filter_get_candidate_rows( $t_page_number, $t_issues_per_page, $t_page_count, $t_issues_count,
	...
	for ( $i = 0; $i < count( $t_issues ); $i++ ) {
		...
		$t_candidate = candidate_get( $row['id'], true );

		$about = $link = $t_path . "view.php?id=" . $row['id'];
		$title = string_rss_links( candidate_format_id( $row['id'] ) . ': ' . $t_candidate->summary );
		
		if ( $row['view_state'] == VS_PRIVATE ) {
			$title .= ' [' . lang_get( 'private' ) . ']';
		}
                
		$description = string_rss_links( $t_candidate->description );

		# subject is category.
		$subject = string_rss_links( $t_candidate->category );

		# optional DC value
		$date = date( 'Y-m-d\TH:i:sO', $t_candidate->last_updated );

		# author of item
		$author = string_rss_links( user_get_name( $t_candidate->reporter_id ) );
		if ( access_has_global_level( config_get( 'show_user_email_threshold' ) ) ) {
			$t_author_email = user_get_field( $t_candidate->reporter_id, 'email' );
		...
		$comments = $t_path . 'view.php?id=' . $row['id'] . '#candidatenotes';
