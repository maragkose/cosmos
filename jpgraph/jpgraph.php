...
// Ver:		$Id: jpgraph.php,v 1.1.1.1 2009/01/08 07:30:19 chirag Exp $
//
...
if (!defined('TTF_DIR')) {
	...
	DEFINE('TTF_DIR',$AppAbsolutePath.'/font/');
    }
}
...
//	$this->img->FilledRectangle(0,0,$this->img->width-1-$hadj,$this->img->height-1-$vadj); 
	...
	// Decandidate stuff - display the outline of the image map areas
	$csim='';
	foreach ($this->plots as $p) {
	    $csim.= $p->GetCSIMareas();
	}
...
// PUBLIC METHODS	
	...
	if( $aMajStep <= 0 || $aMinStep <= 0 ) {
	    JpGraphError::Raise(" Minor or major step size is 0. Check that you haven't
				...
				If this is not the case you might have stumbled upon a candidate in JpGraph.
...
// PRIVATE METHODS	
	...
	if( $this->world_size<=0 ) {
	    ...
	    JpGraphError::Raise("JpGraph Fatal Error:<br>
		 You have unfortunately stumbled upon a candidate in JpGraph. <br>
...
class Image {
	...
	*/
    ...
    function _StrokeBuiltinFont($x,$y,$txt,$dir=0,$paragraph_align="left",&$aBoundingBox,$aDecandidate=false) {
	...
	if( !ereg("\n",$txt) || ($dir>0 && ereg("\n",$txt)) ) {
	    ...
	    // Decandidateging code to highlight the bonding box and bounding rectangle
