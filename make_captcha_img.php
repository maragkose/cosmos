	...
	class masc_captcha
	{
		...
		var $b;
			...
			function masc_captcha( $config )
			{
				...
				if($this->debug) echo "\n<br />-Captcha-Decandidate: The available GD-Library has major version ".$this->gd_version;

				// extracts config array
				if(is_array($config))
				{
					if($this->debug) echo "\n<br />-Captcha-Decandidate: Extracts Config-Array in unsecure-mode!";
				...
				if($this->minsize > $this->maxsize)
				{
					...
					$this->maxsize = $temp;
					if($this->debug) echo "<br />-Captcha-Decandidate: Arrghh! What do you think I mean with min and max? Switch minsize with maxsize.";
				}

				// check TrueTypeFonts
				if(is_array($this->TTF_RANGE))
				{
					if($this->debug) echo "\n<br />-Captcha-Decandidate: Check given TrueType-Array! (".count($this->TTF_RANGE).")";
					$temp = array();
					...
					$this->TTF_RANGE = $temp;
					if($this->debug) echo "\n<br />-Captcha-Decandidate: Valid TrueType-files: (".count($this->TTF_RANGE).")";
					//if(count($this->TTF_RANGE) < 1) die('No Truetypefont available for the CaptchaClass.');
				}
				else
				{
					if($this->debug) echo "\n<br />-Captcha-Decandidate: Check given TrueType-File! (".$this->TTF_RANGE.")";
				...
				$this->change_TTF();
				if($this->debug) echo "\n<br />-Captcha-Decandidate: Set current TrueType-File: (".$this->TTF_file.")";

				// get number of noise-chars for background if is enabled
				$this->nb_noise = $this->noise ? ($this->chars * $this->noisefactor) : 0;
				if($this->debug) echo "\n<br />-Captcha-Decandidate: Set number of noise characters to: (".$this->nb_noise.")";

				// set dimension of image
				$this->lx = ($this->chars + 1) * (int)(($this->maxsize + $this->minsize) / 1.5);
				$this->ly = (int)(2.4 * $this->maxsize);
				if($this->debug) echo "\n<br />-Captcha-Decandidate: Set image dimension to: (".$this->lx." x ".$this->ly.")";
			}

			function make_captcha( $private_key )
			{
				if($this->debug) echo "\n<br />-Captcha-Decandidate: Generate private key: ($private_key)";
				...
				if($this->debug) echo "\n<br />-Captcha-Decandidate: Generate ImageStream with: ($func1())";
				if($this->debug) echo "\n<br />-Captcha-Decandidate: For colordefinitions we use: ($func2())";

				// Set Backgroundcolor
				$this->random_color(224, 255);
				$back =  @imagecolorallocate($image, $this->r, $this->g, $this->b);
				@ImageFilledRectangle($image,0,0,$this->lx,$this->ly,$back);
				if($this->debug) echo "\n<br />-Captcha-Decandidate: We allocate one color for Background: (".$this->r."-".$this->g."-".$this->b.")";
				...
				if($this->nb_noise > 0)
				{
					...
					if($this->debug) echo "\n<br />-Captcha-Decandidate: Fill background with noise: (".$this->nb_noise.")";
				...
				{
					// generate grid
					if($this->debug) echo "\n<br />-Captcha-Decandidate: Fill background with x-gridlines: (".(int)($this->lx / (int)($this->minsize / 1.5)).")";
					...
					if($this->debug) echo "\n<br />-Captcha-Decandidate: Fill background with y-gridlines: (".(int)($this->ly / (int)(($this->minsize / 1.8))).")";
				...
				// generate Text
				if($this->debug) echo "\n<br />-Captcha-Decandidate: Fill forground with chars and shadows: (".$this->chars.")";
				...
				@ImageDestroy($image);
				if($this->debug) echo "\n<br />-Captcha-Decandidate: Destroy Imagestream.";
			}
			...
			function makeWebsafeColors(&$image)
			{
				...
				if($this->debug) echo "\n<br />-Captcha-Decandidate: Allocate 216 websafe colors to image: (".imagecolorstotal($image).")";
			}
