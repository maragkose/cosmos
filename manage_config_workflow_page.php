...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	function parse_workflow( $p_enum_workflow ) {
    	...
    	# prepopulate new candidate state (candidates go from nothing to here)
    	$t_submit_status_array = config_get( 'candidate_submit_status' );
    	$t_new_label = get_enum_to_string( lang_get( 'status_enum_string' ), NEW_ );
    	if ( is_array( $t_submit_status_array ) ) {
    		# @@@ (thraxisp) this is not implemented in candidate_api.php
    	...
    	$t_reopen = config_get( 'candidate_reopen_status' );
	...
	function access_row( ) {
		...
		$t_file_new = config_get_global( 'report_candidate_threshold' );
		$t_global_new = config_get( 'report_candidate_threshold', null, null, ALL_PROJECTS );
		$t_project_new = config_get( 'report_candidate_threshold' );
		...
		foreach ( $t_enum_status as $t_status => $t_status_label) {
		    if ( ! isset( $t_file_set[$t_status] ) ) {
		        $t_file_set[$t_status] = config_get_global('update_candidate_status_threshold');
		    }
		}
		...
		foreach ( $t_enum_status as $t_status => $t_status_label) {
		    if ( ! isset( $t_file_set[$t_status] ) ) {
		        $t_file_set[$t_status] = config_get('update_candidate_status_threshold', null, null, ALL_PROJECTS );
		    }
		}
		...
		foreach ( $t_enum_status as $t_status => $t_status_label) {
		    if ( ! isset( $t_file_set[$t_status] ) ) {
		        $t_file_set[$t_status] = config_get('update_candidate_status_threshold' );
		    }
		}
		...
		foreach ( $t_enum_status as $t_status => $t_status_label) {
			...
			if ( NEW_ == $t_status ) {
				...
				$t_can_change = ( $t_access >= config_get_access( 'report_candidate_threshold' ) );
                $t_colour = '';
                if ( $t_global_new != $t_file_new ) {
                    $t_colour = ' bgcolor="' . $t_colour_global . '" '; # all projects override
                    if ( $t_can_change ) {
                        set_overrides( 'report_candidate_threshold' );
                    }
                }
	...
	if ( ! is_array( config_get( 'candidate_submit_status' ) ) ) {
		threshold_row( 'candidate_submit_status' );
	}
	threshold_row( 'candidate_resolved_status_threshold' );
	threshold_row( 'candidate_reopen_status' );
	threshold_end();
	echo '<br />';
