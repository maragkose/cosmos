...
<?php
	...
	$t_per_page = config_get( 'my_view_candidate_count' );
...
<?php
	...
	while (list ($t_box_title, $t_box_display) = each ($t_boxes)) {
		# don't display candidates that are set as 0
		if ($t_box_display == 0) {
			$t_number_of_boxes = $t_number_of_boxes - 1;
		}

		# don't display "Assigned to Me" candidates to users that candidates can't be assigned to
		else if ( $t_box_title == 'assigned' && ( current_user_is_anonymous() OR user_get_assigned_open_candidate_count( $t_current_user_id, $t_project_id ) == 0 ) ) {
			$t_number_of_boxes = $t_number_of_boxes - 1;
		}

		# don't display "Monitored by Me" candidates to users that can't monitor candidates
		else if ( $t_box_title == 'monitored' && ( current_user_is_anonymous() OR !access_has_project_level( config_get( 'monitor_candidate_threshold' ), $t_project_id, $t_current_user_id ) ) ) {
			$t_number_of_boxes = $t_number_of_boxes - 1;
		}

		# don't display "Reported by Me" candidates to users that can't report candidates
		else if ( in_array( $t_box_title, array( 'reported', 'feedback', 'verify' ) ) && 
				( current_user_is_anonymous() OR !access_has_project_level( config_get( 'report_candidate_threshold' ), $t_project_id, $t_current_user_id ) ) ) {
			$t_number_of_boxes = $t_number_of_boxes - 1;
		}
		# don't display MyInterviews box to Managers level since they do not do first interviews
		else if (( $t_box_title == 'myinterviews' ) && 
				 (user_get_access_level( $t_current_user_id, $t_project_id) == MANAGER  )) {
			$t_number_of_boxes = $t_number_of_boxes - 1;
		}
		# display the box
		else {
