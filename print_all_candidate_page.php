...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------------------------------------------
	# $Id: print_all_candidate_page.php,v 1.89.2.1 2007-10-13 22:34:15 giallu Exp $
	# --------------------------------------------------------
?>
<?php
	...
	# Export :
	#	- the candidates displayed in print_all_candidate_page.php are saved in a .doc or .xls file
...
<?php
	...
	$result = filter_get_candidate_rows( $t_page_number, $t_per_page, $t_page_count, $t_candidate_count );
...
<?php
	...
	#$f_candidate_array stores the number of the selected rows
	#$t_candidate_arr_sort is used for displaying
	#$f_export is a string for the word and excel pages

	$f_candidate_arr = gpc_get_int_array( 'candidate_arr', array() );
	$f_candidate_arr[$row_count]=-1;

	for( $i=0; $i < $row_count; $i++ ) {
		if ( isset( $f_candidate_arr[$i] ) ) {
			$index = $f_candidate_arr[$i];
			$t_candidate_arr_sort[$index]=1;
		}
	}
	$f_export = implode( ',', $f_candidate_arr );
...
<?php
		...
		$t_icons = array(
			array( 'print_all_candidate_page_excel', 'excel', '', 'fileicons/xls.gif', 'Excel 2000' ),
			array( 'print_all_candidate_page_excel', 'html', 'target="_blank"', 'ie.gif', 'Excel View' ),
			array( 'print_all_candidate_page_word', 'word', '', 'fileicons/doc.gif', 'Word 2000' ),
			array( 'print_all_candidate_page_word', 'html', 'target="_blank"', 'ie.gif', 'Word View' ) );
...
<form method="post" action="print_all_candidate_page.php">
...
<tr>
	<td class="form-title" colspan="<?php echo $t_num_of_columns / 2 + $t_num_of_columns % 2; ?>">
		<?php echo lang_get( 'viewing_candidates_title' ) ?>
		<?php
			if ( $row_count > 0 ) {
				$v_start = $f_offset+1;
	...
	<td class="right" colspan="<?php echo $t_num_of_columns / 2 ?>">
		<?php # print_bracket_link( 'print_all_candidate_options_page.php', lang_get( 'printing_options_link' ) ) ?>
		<?php # print_bracket_link( 'view_all_candidate_page.php', lang_get( 'view_candidates_link' ) ) ?>
...
<?php
	for( $i=0; $i < $row_count; $i++ ) {
		...
		if ( isset( $t_candidate_arr_sort[ $t_row['id'] ] ) || ( $t_show_flag==0 ) ) {
?>
