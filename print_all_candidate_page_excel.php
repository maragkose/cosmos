...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	# --------------------------------------------------------
	# $Id: print_all_candidate_page_excel.php,v 1.56.2.1 2007-10-13 22:34:16 giallu Exp $
	# --------------------------------------------------------
?>
<?php
	# Excel (97 and above) export page
	# The candidates displayed in print_all_candidate_page.php are saved in a .xls file
...
?>
<?php require( 'print_all_candidate_options_inc.php' ) ?>
...
<?php
	...
	$t_user_print_pref_table = config_get( 'cosmos_user_print_pref_table' );
...
<?php
	...
	$f_candidate_arr = explode_enum_string( $f_export );

	# @@decandidate var_dump($t_field_name_arr);
	for( $i=0; $i < $row_count; $i++ ) {

		# prefix candidate data with v_
		extract( $result[$i], EXTR_PREFIX_ALL, 'v' );

		if ( in_array( $v_id, $f_candidate_arr ) || ( $f_show_flag==0 ) ) {
            $t_last_updated = date( $g_short_date_format, $v_last_updated );

            # grab the candidatenote count
            $candidatenote_count = candidate_get_candidatenote_count( $v_id );
...
<tr>
	...
	<?php if ( ( $name_index < $field_name_count ) && ( !isset( $t_prefs[$name_index] ) || ( 1 == $t_prefs[$name_index] ) ) ) { ?>
	<td class=xl2216681 nowrap style='border-top:none;border-left:none'>
	<?php      if ( access_has_candidate_level( config_get( 'view_handler_threshold' ), $v_id ) ) {
...
<td colspan="3">
<?php  # print candidatenotes
		# get the candidatenote data
 		if ( !access_has_candidate_level( config_get( 'private_candidatenote_threshold' ), $v_id ) ) {
		...
		$query6 = "SELECT *
				...
				WHERE candidate_id='$v_id' $t_restriction
				ORDER BY date_submitted $t_candidatenote_order";
		$result6 = db_query( $query6 );
		$num_notes = db_num_rows( $result6 );

		# save the index, and use an own candidatenote_index
		$candidatenote_index = $name_index ;
		...
		for ( $k=0; $k < $num_notes; $k++ ) {
			...
			$v3_candidatenote_text_id = db_result( $result7, 0, 1 );
			$t_note = '';
...
<table>
<tr>
	<?php if ( ( $candidatenote_index < $field_name_count ) && ( !isset( $t_prefs[$candidatenote_index] )||( 1 == $t_prefs[$candidatenote_index] ) ) ) { ?>
	<td class=xl2216681 nowrap style='border-top:none;border-left:none'>
