...
# along with COSMOS.  If not, see <http://www.gnu.org/licenses/>.
	...
	foreach( $t_project_ids as $t_project_id ) {
		...
		$t_project_name = project_get_field( $t_project_id, 'name' );
		$t_can_view_private = access_has_project_level( config_get( 'private_candidate_threshold' ), $t_project_id );
		...
		$t_relation_table = config_get( 'cosmos_candidate_relationship_table' );
		...
		foreach( $t_version_rows as $t_version_row ) {
			...
			$query = "SELECT sbt.*, $t_relation_table.source_candidate_id, dbt.target_version as parent_version FROM $t_candidate_table AS sbt
						LEFT JOIN $t_relation_table ON sbt.id=$t_relation_table.destination_candidate_id AND $t_relation_table.relationship_type=2
						LEFT JOIN $t_candidate_table AS dbt ON dbt.id=$t_relation_table.source_candidate_id
			...
			while ( $t_row = db_fetch_array( $t_result ) ) {
				# hide private candidates if user doesn't have access to view them.
				if ( !$t_can_view_private && ( $t_row['view_state'] == VS_PRIVATE ) ) {
					continue;
				}

				candidate_cache_database_result( $t_row );

				# check limit_Reporter (Issue #4770)
				# reporters can view just issues they reported
				if ( ON === $t_limit_reporters && $t_user_access_level_is_reporter &&
					 !candidate_is_user_reporter( $t_row['id'], $t_user_id )) {
					continue;
				}

				$t_issue_id = $t_row['id'];
				$t_issue_parent = $t_row['source_candidate_id'];
				...
				if ( !isset( $t_issues_counted[$t_issue_id] ) ) {
					...
					if ( candidate_is_resolved( $t_issue_id ) ) {
						$t_issues_resolved++;
					}
