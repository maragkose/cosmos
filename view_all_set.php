...
<?php
	...
	if ( is_array( gpc_get( 'fixed_in_version', null ) ) ) {
		$f_fixed_in_version = gpc_get_string_array( 'handler_id', META_FILTER_ANY );
	} else {
		$f_fixed_in_version = gpc_get_string( 'handler_id', META_FILTER_ANY );
	...
	$f_relationship_candidate = gpc_get_int( 'relationship_candidate', 0 );
	...
	# show candidates per page
	if ( $f_per_page < 0 ) {
		$f_per_page = config_get( 'default_limit_view' );
	}

	# combine sort settings
	#  (f_sort overrides f_sort_1 if set to keep old sorting code working in view_all_candidate_inc)
...
*/
	...
	switch ( $f_type ) {
		...
		case '1':
				...
				$t_setting_arr['relationship_candidate'] = $f_relationship_candidate;
		...
		case '4':
				...
				$t_setting_arr['relationship_candidate'] = 0;
	...
	if ( $f_print ) {
		$t_redirect_url = 'print_all_candidate_page.php';
	} else {
